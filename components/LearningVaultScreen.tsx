...
export default function LearningVaultScreen({ kind }: { kind: VaultKind }) {
  ...
  const deleteItem = (item: LearningVaultItem) => {
    Alert.alert(title, `Remove "${item.text}" from ${title}?`, [
      ...
      { text: wt('vault-delete'), style: 'destructive', onPress: () => writeItems(items.filter(entry => entry.text !== item.text)).then(() => pushVaults()) },
    ]);
  };
...
<div class="footer"><p>Generated by Poly-Puff ESL Trainer · poly-puff.com</p><p>Developed by Mark Middleton, Cape Town, South Africa</p></div>
</body></html>`;
