...
export default function WordChunksScreen() {
  ...
  const wordChunksTitle = wt('word-chunks');
  const scoreLabels = {
    excellent: ui('scoreExcellent', 'Excellent'),
    good: ui('scoreGood', 'Good'),
    ok: ui('scoreOk', 'OK'),
    practice: t.practice,
  };
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        ...
        <ScrollView ref={scrollRef} contentContainerStyle={ds.scroll} keyboardShouldPersistTaps="handled">
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ flexDirection: 'row', marginBottom: 12 }}
            accessibilityRole="tablist" accessibilityLabel={ui('selectCefrLevel', 'Select CEFR level')}>
            {LEVELS.map(l => (
              <TouchableOpacity
                key={l}
          ...
          {sessionCount > 0 && (
            ...
            >
              <View style={ds.statPill}>
                ...
                <Text style={[ds.statText, { color: C.amber || '#FFBE0B' }]}>{avgScore}% {ui('averageAbbrev', 'avg')}</Text>
              </View>
              <View style={ds.statPill}>
                <BookOpen size={12} color={C.cyan || '#00E5FF'} />
                <Text style={[ds.statText, { color: C.cyan || '#00E5FF' }]}>{sessionCount} {ui('chunksDone', 'done')}</Text>
              </View>
            </View>
          )}
            ...
            <View style={{ alignItems: 'center', paddingHorizontal: 8, paddingTop: 8, paddingBottom: 24 }}>
              <View style={{
                ...
                <TouchableOpacity
                  ...
                  accessibilityRole="button"
                  accessibilityLabel={wt('start-session')}
                >
          ...
          {/* ── CHUNK CARD ── */}
          {loading ? (
            <View style={ds.loadingCard} accessibilityLabel={`${ui('generatingChunk', 'Generating chunk')} in ${nativeLanguage}`}>
              <ActivityIndicator size="large" color={C.cyan || '#00E5FF'} />
              <Text style={ds.loadingText}>{ui('generatingChunk', 'Generating chunk')} in {nativeLanguage}...</Text>
            </View>
          ) : chunk ? (
          ...
          {sessionStarted && chunk && !result && !loading && (
            <View style={ds.inputSection}>
              ...
              <TextInput
                style={ds.input}
                placeholder={ui('englishTranslationPlaceholder', 'Type your English translation...')}
              ...
              <View style={ds.inputActions}>
                ...
                >
                  {checking
                    ...
                    : <><Send size={16} color="#000" /><Text style={ds.submitText}>{wt('check-answer')}</Text></>
                  }
                </TouchableOpacity>
              </View>
            </View>
            ...
            <Animated.View style={[ds.resultCard, {
              ...
              >
                ...
                <Text style={{ fontSize: scaledFont(9), fontWeight: '700', color: scoreColor(result.score, C), marginTop: 1 }}>
                  {scoreLabel(result.score, scoreLabels)}
                </Text>
              </View>
              ...
              >
                ...
                <TouchableOpacity
                  onPress={() => Speech.speak(result.correct, { language: 'en-US', rate: 0.9 })}
                  accessibilityRole="button" accessibilityLabel={ui('hearCorrectAnswer', 'Hear correct answer')}
