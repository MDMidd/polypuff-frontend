/**
 * Word Chunks Trainer - Poly-Puff v7 (Accessibility Update)
 * ===========================================================
 *
 * ACCESSIBILITY CHANGES:
 *   1.  Custom back button: accessibilityRole="button" + "Go back" label
 *   2.  Level pills: accessibilityRole="tab" + selected state in tablist
 *   3.  Category pills: accessibilityRole="tab" + selected state in tablist
 *   4.  Chunk native text: accessibilityLabel "Translate this chunk: ..."
 *   5.  Listen button: accessibilityRole="button" + label + hint
 *   6.  Input field: accessibilityLabel + hint
 *   7.  "Show answer" button: accessibilityRole="button" + label
 *   8.  Submit button: accessibilityRole="button" + disabled state + opacity 0.6
 *   9.  Score ring: accessibilityLabel with spoken score
 *   10. Correct answer row: accessibilityLabel
 *   11. Hear correct button: accessibilityRole="button" + label
 *   12. Tip box: accessibilityLabel
 *   13. announce() after score calculation
 *   14. announce() after new chunk generated
 *   15. Session stats: grouped accessibilityLabel
 *   16. Category badge: accessibilityLabel
 *   17. All key font sizes wrapped in scaledFont()
 *   18. Minimum 44×44dp touch targets
 *   19. Disabled opacity raised to 0.6
 *
 * FILE: app/wordchunks.tsx
 * REPLACES: D:\Project\MyProject\translation-trainer-frontend\app\wordchunks.tsx
 */
...
function scoreColor(score, C) {
  if (score >= 90) return C.emerald || '#00E5A0';
  if (score >= 70) return C.cyan   || '#00E5FF';
  if (score >= 50) return C.amber  || '#FFBE0B';
  return C.red || '#FF4D6D';
}

// ✅ NEW: Text label for score
...
export default function WordChunksScreen() {
  ...
  const [nativeLanguage, setNativeLang] = useState('');
  ...
  useFocusEffect(useCallback(() => {
    ...
    return () => {
      if (bgTimerStartRef.current !== null) {
        ...
        if (elapsed > 2) recordExerciseTime('word_chunks', 'Word Chunks', elapsed);
  ...
  const checkAnswer = async () => {
    ...
    try {
      ...
      const finalData = hintUsed && finalScore < rawScore
  ...
  const saveWordChunkProgress = async (sc, feedbackText, chunkData) => {
    const category = chunkData?.category || 'Vocabulary';
    await recordModuleProgress({
      exerciseId: 'word_chunks',
      score: sc,
      detail: chunkData?.native?.substring(0, 60) || '',
      feedback: feedbackText,
      weakAreas: sc < 70
        ? [{ category, description: `Practice ${category} word chunks` }]
        : [],
    });
  };
  ...
  const revealAnswer = () => {
    ...
    announce(`Answer revealed: ${chunk.english}`);
  };
    ...
    <ScreenBackground>
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        ...
        <ScrollView contentContainerStyle={ds.scroll} keyboardShouldPersistTaps="handled">
          ...
          {/* ✅ A11Y: Session stats with grouped label */}
          {sessionCount > 0 && (
            <View
              style={ds.statsRow}
              accessibilityRole="text"
              accessibilityLabel={`Session: ${avgScore} percent average, ${sessionCount} chunks done`}
            >
              <View style={ds.statPill}>
                <Star size={12} color={C.amber || '#FFBE0B'} />
                <Text style={[ds.statText, { color: C.amber || '#FFBE0B' }]}>{avgScore}% avg</Text>
              </View>
              <View style={ds.statPill}>
                <BookOpen size={12} color={C.cyan || '#00E5FF'} />
                <Text style={[ds.statText, { color: C.cyan || '#00E5FF' }]}>{sessionCount} done</Text>
              </View>
            </View>
          )}

          {/* ✅ A11Y: Category selector with tablist */}
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 14 }}
            accessibilityRole="tablist" accessibilityLabel="Select chunk category">
            {[
          ...
          {!sessionStarted && !loading && (
            <View style={{ alignItems: 'center', paddingHorizontal: 8, paddingTop: 8, paddingBottom: 24 }}>
              <View style={{
                backgroundColor: C.card || '#1F2937', borderRadius: 20,
                padding: 24, width: '100%',
                borderWidth: 1, borderColor: (C.cyan || '#00E5FF') + '30',
                alignItems: 'center',
              }}>
                <Text style={{ fontSize: 44, marginBottom: 12 }}>🧩</Text>
                <Text style={{ fontSize: scaledFont(20), fontWeight: '800', color: C.text, marginBottom: 8, textAlign: 'center' }}>Word Chunks</Text>
                <Text style={{ fontSize: scaledFont(13), color: C.textMuted, textAlign: 'center', lineHeight: 20, marginBottom: 20 }}>
                  Translate short phrases and expressions from {nativeLanguage} into English.{'\n'}Select a category, then tap Start.
                </Text>
                <TouchableOpacity
                  onPress={() => { setSessionStarted(true); generateChunk(); }}
                  style={{
          ...
          {/* ── CHUNK CARD ── */}
          {loading ? (
            <View style={ds.loadingCard} accessibilityLabel={`Generating chunk in ${nativeLanguage}`}>
              <ActivityIndicator size="large" color={C.cyan || '#00E5FF'} />
              <Text style={ds.loadingText}>Generating chunk in {nativeLanguage}…</Text>
            </View>
          ) : chunk ? (
            <Animated.View style={[ds.chunkCard, {
              opacity: cardAnim,
              transform: [{ scale: cardAnim.interpolate({ inputRange: [0, 1], outputRange: [0.92, 1] }) }],
            }]}>
              {/* ✅ A11Y: Category badge */}
              <View style={ds.categoryBadge} accessibilityLabel={`Category: ${chunk.category || 'Expression'}`}>
                <Text style={ds.categoryText}>{chunk.category?.toUpperCase() || 'EXPRESSION'}</Text>
              </View>

              {/* ✅ A11Y: Native phrase with descriptive label */}
              <Text
                style={ds.chunkNative}
                accessibilityRole="text"
                accessibilityLabel={`Translate this chunk: ${chunk.native}`}
              >
                {chunk.native}
              </Text>
              <Text style={ds.chunkHint}>Translate this phrase into English</Text>
              ...
              >
                ...
                <Text style={ds.listenText}>Hear in {nativeLanguage}</Text>
              </TouchableOpacity>
            </Animated.View>
          ) : null}

          {/* ── INPUT ── */}
          {sessionStarted && chunk && !result && !loading && (
            <View style={ds.inputSection}>
              ...
              <TextInput
                ...
                accessibilityLabel="Your English translation of the chunk"
              ...
              <View style={ds.inputActions}>
                ...
                {!hintUsed && (
                  <TouchableOpacity
                    onPress={() => {
                      const answer = chunk.english || '';
                      const words = answer.trim().split(' ');
                      const preview = words.length === 1
                        ? answer.substring(0, Math.ceil(answer.length * 0.4)) + '...'
                        : words.slice(0, Math.max(1, Math.ceil(words.length * 0.3))).join(' ') + '...';
                      setHintText(preview);
                      setHintUsed(true);
                    }}
                    style={[ds.revealBtn, {
                      backgroundColor: 'rgba(255,190,11,0.12)',
                      borderColor: 'rgba(255,190,11,0.35)', borderWidth: 1,
                    }]}
                    accessibilityRole="button"
                  ...
                  >
                    ...
                    <Text style={[ds.revealText, { color: '#FFBE0B' }]}>Hint (-20)</Text>
                  </TouchableOpacity>
                )}
                {hintUsed && hintText ? (
                  <View style={[ds.revealBtn, {
                ...
                {/* ✅ A11Y: Submit button */}
                <TouchableOpacity
                  style={[ds.submitBtn, { opacity: input.trim() ? 1 : 0.6 }]}
                  onPress={checkAnswer}
                  disabled={!input.trim() || checking}
                  accessibilityRole="button"
                  accessibilityLabel="Check answer"
                  accessibilityState={{ disabled: !input.trim() || checking, busy: checking }}
                >
                  {checking
                    ? <ActivityIndicator size="small" color="#000" />
                    : <><Send size={16} color="#000" /><Text style={ds.submitText}>Check</Text></>
                  }
                </TouchableOpacity>
              </View>
            </View>
          ...
          {/* ── RESULT CARD ── */}
          {sessionStarted && result && chunk && (
            <Animated.View style={[ds.resultCard, {
              borderColor: scoreColor(result.score, C) + '60',
              opacity: resultAnim,
              transform: [{ scale: resultAnim.interpolate({ inputRange: [0, 1], outputRange: [0.94, 1] }) }],
            }]}>
              {/* ✅ A11Y: Score ring with spoken label + text grade */}
              <View
                style={[ds.scoreRing, { borderColor: scoreColor(result.score, C) }]}
                accessibilityRole="text"
                accessibilityLabel={scoreAnnouncement(result.score)}
              >
                ...
                {/* ✅ NEW: Text grade label */}
                <Text style={{ fontSize: scaledFont(9), fontWeight: '700', color: scoreColor(result.score, C), marginTop: 1 }}>
                  {scoreLabel(result.score)}
                </Text>
              </View>

              {/* ✅ A11Y: Correct answer row */}
              <View
                style={ds.correctRow}
                accessibilityRole="text"
                accessibilityLabel={`Correct answer: ${result.correct}`}
              >
                <CheckCircle size={16} color={C.emerald || '#00E5A0'} />
                <TouchableOpacity
                  onPress={() => Speech.speak(result.correct, { language: 'en-US', rate: 0.9 })}
                ...
                <TouchableOpacity
                  onPress={() => Speech.speak(result.correct, { language: 'en-US', rate: 0.9 })}
                  accessibilityRole="button" accessibilityLabel="Hear correct answer"
                  style={{ minWidth: 44, minHeight: 44, alignItems: 'center', justifyContent: 'center' }}
                >
                  <Volume2 size={14} color={C.cyan || '#00E5FF'} />
                </TouchableOpacity>
                <TouchableOpacity
                  onPress={() => { setVaultPhrase(result.correct); setVaultModal(true); }}
                  accessibilityRole="button" accessibilityLabel="Save to Vocabulary Vault"
                  style={{ minWidth: 44, minHeight: 44, alignItems: 'center', justifyContent: 'center' }}
                >
              ...
              {/* ✅ A11Y: Tip box */}
              {chunk.tip ? (
                <View style={ds.tipBox} accessibilityLabel={`Tip: ${chunk.tip}`}>
                  <Zap size={14} color={C.amber || '#FFBE0B'} />
                  <Text style={ds.tipText}>{chunk.tip}</Text>
                </View>
              ) : null}
              ...
              {/* ✅ A11Y: Next button */}
              <TouchableOpacity
                style={[ds.nextBtn, { backgroundColor: C.cyan || '#00E5FF' }]}
      ...
      <Modal visible={vaultModal} transparent animationType="fade" onRequestClose={() => setVaultModal(false)}>
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(0,0,0,0.65)', padding: 24 }}>
          <View style={{ backgroundColor: C.card || '#1F2937', borderRadius: 16, padding: 20, width: '100%', maxWidth: 380, borderWidth: 1, borderColor: (C.cyan || '#00E5FF') + '40' }}>
            <Text style={{ fontSize: scaledFont(16), fontWeight: '800', color: C.text, marginBottom: 4 }}>Save to Vault 💾</Text>
            <Text style={{ fontSize: scaledFont(12), color: C.textMuted, marginBottom: 14 }}>Tap to save to your Vocabulary Vault:</Text>
            <TouchableOpacity
            ...
            >
              <Text style={{ fontSize: scaledFont(10), color: C.textMuted, marginBottom: 2, letterSpacing: 0.8 }}>FULL PHRASE</Text>
            ...
            {vaultPhrase.trim().split(/\s+/).filter(w => w.replace(/[.,!?;:"'()]/g, '').length > 2).map((word, i) => (
              <TouchableOpacity key={i}
                ...
                style={{ backgroundColor: (C.cyan || '#00E5FF') + '12', borderRadius: 10, padding: 12, marginBottom: 8, borderWidth: 1, borderColor: (C.cyan || '#00E5FF') + '30' }}
              >
                <Text style={{ fontSize: scaledFont(10), color: C.textMuted, marginBottom: 2, letterSpacing: 0.8 }}>WORD</Text>
                <Text style={{ fontSize: scaledFont(14), fontWeight: '700', color: C.cyan || '#00E5FF' }}>{word.replace(/[.,!?;:"'()]/g, '')}</Text>
              </TouchableOpacity>
            ))}
            <TouchableOpacity onPress={() => setVaultModal(false)} style={{ alignItems: 'center', paddingVertical: 12, marginTop: 4 }}>
              <Text style={{ fontSize: scaledFont(13), color: C.textMuted }}>Cancel</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
...
function dynamicStyles(C) {
  return StyleSheet.create({
    ...
    levelPillActive: { backgroundColor: (C.emerald || '#10B981') + '20', borderColor: C.emerald || '#10B981' },
    levelPillText: { fontSize: scaledFont(13), fontWeight: '600', color: C.textMuted },
    levelPillTextActive: { color: C.emeraldLight || '#34D399' },
    levelBadge:    { paddingHorizontal: 14, paddingVertical: 6, borderRadius: 10, backgroundColor: (C.cyan || '#00E5FF') + '20', borderWidth: 1, borderColor: (C.cyan || '#00E5FF') + '40' },
    levelText:     { fontSize: scaledFont(14), fontWeight: '800', color: C.cyan || '#00E5FF' },
    statsRow:      { flexDirection: 'row', justifyContent: 'center', gap: 12, marginBottom: 12 },
    statPill:      { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: 'rgba(255,255,255,0.06)', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 20 },
    statText:      { fontSize: scaledFont(12), fontWeight: '700' },
    loadingCard:   { backgroundColor: C.card, borderRadius: 20, padding: 40, marginBottom: 16, alignItems: 'center', gap: 16 },
    loadingText:   { fontSize: scaledFont(14), color: C.textMuted },
    chunkCard:     { backgroundColor: C.card, borderRadius: 20, padding: 24, marginBottom: 16, borderWidth: 1, borderColor: (C.cyan || '#00E5FF') + '20', alignItems: 'center' },
    categoryBadge: { backgroundColor: (C.purple || '#B06CFF') + '20', paddingHorizontal: 12, paddingVertical: 4, borderRadius: 20, marginBottom: 16 },
    ...
    revealBtn:     { paddingHorizontal: 14, paddingVertical: 10, borderRadius: 12, backgroundColor: 'rgba(255,255,255,0.06)', minHeight: 44, justifyContent: 'center' },
    revealText:    { fontSize: scaledFont(13), color: C.textMuted },
    submitBtn:     { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: C.cyan || '#00E5FF', paddingHorizontal: 20, paddingVertical: 10, borderRadius: 12, minHeight: 44 },
    submitText:    { fontSize: scaledFont(14), fontWeight: '700', color: '#000' },
    resultCard:    { backgroundColor: C.card, borderRadius: 20, padding: 24, marginBottom: 16, borderWidth: 1, alignItems: 'center' },
    scoreRing:     { width: 90, height: 90, borderRadius: 45, borderWidth: 4, alignItems: 'center', justifyContent: 'center', marginBottom: 16 },
    scoreNumber:   { fontSize: scaledFont(28), fontWeight: '900' },
    scoreLabel:    { fontSize: scaledFont(11), marginTop: -4 },
    correctRow:    { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: (C.emerald || '#00E5A0') + '15', paddingHorizontal: 16, paddingVertical: 10, borderRadius: 12, marginBottom: 12, width: '100%' },
    correctText:   { fontSize: scaledFont(16), fontWeight: '700', color: C.emerald || '#00E5A0', flex: 1 },
    tipBox:        { flexDirection: 'row', alignItems: 'flex-start', gap: 8, backgroundColor: (C.amber || '#FFBE0B') + '15', padding: 12, borderRadius: 12, marginBottom: 12, width: '100%' },
    tipText:       { fontSize: scaledFont(13), color: C.amber || '#FFBE0B', flex: 1, lineHeight: 19 },
    feedbackText:  { fontSize: scaledFont(13), color: C.textSec, textAlign: 'center', marginBottom: 16, lineHeight: 20 },
    nextBtn:       { flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: 28, paddingVertical: 13, borderRadius: 14, marginTop: 4, minHeight: 44 },
    nextText:      { fontSize: scaledFont(15), fontWeight: '700', color: '#000' },
    pickerOverlay: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'flex-end' },
    pickerCard:    { borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24 },
    pickerTitle:   { fontSize: scaledFont(18), fontWeight: '800', marginBottom: 16, textAlign: 'center' },
    levelRow:      { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingVertical: 14, paddingHorizontal: 12, borderRadius: 10, marginBottom: 4 },
    levelRowText:  { fontSize: scaledFont(16), fontWeight: '600' },
  });
}
