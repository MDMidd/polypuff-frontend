/**
 * Writing Mode Screen - Poly-Puff v7 (Accessibility Update)
 * ===========================================================
 *
 ...
 *   3.  Prompt pills: accessibilityRole="tab" + selected state in tablist
 *   4.  Length buttons: accessibilityRole="tab" + selected state
 *   5.  Writing TextInput: accessibilityLabel with topic + word target
 *   6.  Word counter: accessibilityLabel with progress info
 *   7.  Clear button: accessibilityRole="button" + label
 *   8.  Submit button: accessibilityRole="button" + disabled state + opacity 0.6
 *   9.  Score ring: accessibilityLabel with spoken score + text grade
 *   10. Grammar corrections: accessibilityLabel per correction
 *   11. Improved version box: accessibilityLabel
 *   12. Write Again button: accessibilityRole="button" + label
 *   13. announce() after scoring
 *   14. Session stats: grouped accessibilityLabel
 *   15. Section labels: accessibilityRole="header"
 *   16. All key font sizes wrapped in scaledFont()
 *   17. Minimum 44×44dp touch targets
 *   18. Disabled opacity raised to 0.6
 *
 * FILE: app/writing.tsx
 * REPLACES: D:\Project\MyProject\translation-trainer-frontend\app\writing.tsx
 */
...
const WORD_TARGETS = [
  { label: 'Short (30-50 words)',   min: 30,  max: 50 },
  { label: 'Medium (50-100 words)', min: 50,  max: 100 },
  { label: 'Long (100-200 words)',  min: 100, max: 200 },
];
...
export default function WritingScreen() {
  ...
  const [nativeLanguage, setNativeLang] = useState('Spanish');
  ...
  useFocusEffect(useCallback(() => {
    ...
    return () => {
      if (bgTimerStartRef.current !== null) {
        ...
        if (elapsed > 2) recordExerciseTime('writing', 'Writing', elapsed);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await saveWritingProgress(data.score ?? 0, data.feedback || '', prompt.label);
      // ✅ NEW: Announce score
      ...
      const score = Math.min(100, Math.round(lengthScore * 0.7 + varietyBonus + 20));

      const corrections = [];
      if (wordCount < target.min) corrections.push(`Try to write at least ${target.min} words (you wrote ${wordCount}).`);
      if (sentenceCount < 2) corrections.push('Try to write more than one sentence for better structure.');
      if (!/[.!?]$/.test(text.trim())) corrections.push('End your writing with proper punctuation (. ! ?).');

      const feedback = score >= 80
        ? 'Good work! Your writing meets the length target and shows effort.'
        : score >= 50
        ? 'Nice start! Try to expand your ideas and use more varied sentence structures.'
        : 'Keep practicing! Try to write longer, more detailed responses.';
      ...
      Animated.spring(resultAnim, { toValue: 1, useNativeDriver: true, friction: 6 }).start();
      await saveWritingProgress(score, feedback, prompt.label);
      // ✅ NEW: Announce fallback score
      announce(scoreAnnouncement(score));
    }
    setLoading(false);
  };

  const saveWritingProgress = async (sc, feedbackText, promptLabel) => {
    await recordModuleProgress({
      exerciseId: 'writing',
      score: sc,
      detail: `${promptLabel} - ${wordCount} words`,
      feedback: feedbackText,
      weakAreas: sc < 70
        ? [{ category: 'Writing', description: 'Practice free-form writing' }]
        : [],
    });
  };
        ...
        <ScrollView ref={scrollRef} contentContainerStyle={ds.scroll} keyboardShouldPersistTaps="handled">
          ...
          {sessionCount > 0 && (
            <View
              ...
              accessibilityLabel={`Session: ${avgScore} percent average, ${sessionCount} writings done`}
            >
          ...
          <View style={{ flexDirection: 'row', gap: 8, marginBottom: 16 }}>
            {WORD_TARGETS.map((t, i) => (
              <TouchableOpacity
                key={i}
                style={[ds.lengthBtn, selectedLength === i && ds.lengthBtnActive]}
                onPress={() => setSelectedLength(i)}
                {...a11yTab(`${t.label}`, selectedLength === i)}
              >
                <Text style={[ds.lengthText, selectedLength === i && { color: C.emerald || '#00E5A0' }]}>{t.label}</Text>
              </TouchableOpacity>
            ))}
          </View>
          ...
          {/* ── WRITING AREA ── */}
          {!result && (
            <View style={ds.writingCard}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8 }}>
                <Text style={ds.promptHint}>{prompt.desc}</Text>
                {/* ✅ A11Y: Word counter with label */}
                <Text
                  style={[ds.wordCounter, { color: wordCount >= target.min ? (C.emerald || '#00E5A0') : (C.textMuted) }]}
                  accessibilityRole="text"
                  accessibilityLabel={`${wordCount} words written. Target: ${target.min} to ${target.max} words. ${wordCount >= target.min ? 'Target reached.' : `${target.min - wordCount} more needed.`}`}
                >
              ...
              <View style={ds.writingActions}>
                ...
                {/* ✅ A11Y: Submit button */}
                <TouchableOpacity
                  style={[ds.submitBtn, { opacity: wordCount >= 5 && !loading ? 1 : 0.6 }]}
                  onPress={handleSubmit}
                  disabled={wordCount < 5 || loading}
                  accessibilityRole="button"
                  accessibilityLabel={loading ? 'Submitting writing for grading' : 'Submit writing for AI grading'}
                  accessibilityState={{ disabled: wordCount < 5 || loading, busy: loading }}
                >
                  {loading
                    ? <ActivityIndicator size="small" color="#000" />
                    : <><Send size={16} color="#000" /><Text style={ds.submitText}>Submit</Text></>
                  }
                </TouchableOpacity>
              </View>
            </View>
          ...
          {result && (
            ...
            >
                ...
                <View style={ds.correctionsBox}>
                  ...
                  {result.corrections.map((c, i) => (
                    <View key={i} style={ds.correctionRow}>
                      <Text
                        style={ds.correctionText}
                        accessibilityLabel={`Grammar note ${i + 1}: ${c}`}
                      >
                        • {c}
                      </Text>
                    </View>
              ...
              {/* ✅ A11Y: Write Again button */}
              <TouchableOpacity
                style={[ds.nextBtn, { backgroundColor: C.cyan || '#00E5FF' }]}
                onPress={handleReset}
                accessibilityRole="button"
                accessibilityLabel="Write again"
                accessibilityHint="Clears the result and lets you write a new piece"
              >
                <Text style={ds.nextText}>Write Again</Text>
...
function dynamicStyles(C) {
  return StyleSheet.create({
    ...
    sectionLabel:  { fontSize: scaledFont(13), fontWeight: '600', color: C.textSec || '#94A3B8', marginBottom: 8 },

    promptPill:       { paddingHorizontal: 14, paddingVertical: 10, borderRadius: 12, marginRight: 8, backgroundColor: C.card, borderWidth: 1, borderColor: (C.border || '#475569') + '30', minHeight: 44 },
    promptPillActive: { backgroundColor: (C.cyan || '#00E5FF') + '15', borderColor: (C.cyan || '#00E5FF') + '50' },
    promptLabel:      { fontSize: scaledFont(13), fontWeight: '600', color: C.text },
    promptDesc:       { fontSize: scaledFont(11), color: C.textMuted, marginTop: 2 },

    lengthBtn:       { flex: 1, paddingVertical: 12, paddingHorizontal: 6, borderRadius: 10, alignItems: 'center', justifyContent: 'center', backgroundColor: C.card, borderWidth: 1, borderColor: (C.border || '#475569') + '30', minHeight: 44 },
    lengthBtnActive: { backgroundColor: (C.emerald || '#00E5A0') + '20', borderColor: (C.emerald || '#00E5A0') + '50' },
    lengthText:      { fontSize: scaledFont(12), fontWeight: '600', color: C.textMuted, textAlign: 'center' },

    writingCard:   { backgroundColor: C.card, borderRadius: 20, padding: 16, borderWidth: 1, borderColor: (C.cyan || '#00E5FF') + '20', marginBottom: 16 },
    promptHint:    { fontSize: scaledFont(13), color: C.textMuted },
    wordCounter:   { fontSize: scaledFont(13), fontWeight: '700' },
    textArea:      { minHeight: 180, fontSize: scaledFont(16), color: C.text, lineHeight: 24, padding: 4, textAlignVertical: 'top' },
    writingActions: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 12, borderTopWidth: 1, borderTopColor: (C.border || '#475569') + '20', paddingTop: 12 },
    clearBtn:      { flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 14, paddingVertical: 10, borderRadius: 12, backgroundColor: 'rgba(255,255,255,0.06)', minHeight: 44 },
    clearText:     { fontSize: scaledFont(13), color: C.textMuted },
    ...
    feedbackText:  { fontSize: scaledFont(13), color: C.textSec || '#94A3B8', textAlign: 'center', marginBottom: 16, lineHeight: 20 },

    correctionsBox:   { backgroundColor: (C.amber || '#FFBE0B') + '10', borderRadius: 12, padding: 14, width: '100%', marginBottom: 12 },
    correctionsTitle: { fontSize: scaledFont(13), fontWeight: '700', color: C.amber || '#FFBE0B' },
    correctionRow:    { marginTop: 4 },
    correctionText:   { fontSize: scaledFont(12), color: C.textSec || '#94A3B8', lineHeight: 18 },

    improvedBox:   { backgroundColor: (C.emerald || '#00E5A0') + '10', borderRadius: 12, padding: 14, width: '100%', marginBottom: 16 },
    improvedTitle: { fontSize: scaledFont(13), fontWeight: '700', color: C.emerald || '#00E5A0' },
    improvedText:  { fontSize: scaledFont(13), color: C.text, lineHeight: 20 },
