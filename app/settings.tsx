/**
 * Settings Screen - Poly-Puff v6.3
 * =================================================
 *
 * CHANGES IN v6.3:
 *   - Added DigestSettings component (Weekly Digest section)
 *   - Added Mail icon import
 *
 * FILE: app/settings.tsx
 */

import React, { useState, useEffect, useCallback, useMemo, type ReactNode } from 'react';
import { useRouter, useFocusEffect } from 'expo-router';
import {
  Image, View, Text, ScrollView, TouchableOpacity,
  Switch, Alert, Linking, Modal, TextInput, Platform, ActivityIndicator, type GestureResponderEvent,
} from 'react-native';
import AsyncStorage from '@react-native-async-storage/async-storage';
import Constants from 'expo-constants';
import {
  Volume2, Trash2, Info, ExternalLink, ChevronRight,
  Download, Shield, RotateCcw, Bell, Mail, Server,
  User, KeyRound, HelpCircle, MessageSquare, CreditCard, RefreshCw, LogOut,
  Globe, Languages, BookOpen, CheckCircle, X, Sparkles, Trophy, Pencil,
} from 'lucide-react-native';
import { useTheme } from '../contexts/ThemeContext';
import { useLanguage } from '../contexts/LanguageContext';
import type { LangCode } from '../contexts/translations';
import { LANGUAGE_OPTIONS, flagForLangCode, flagForEnglishName, isRtlLanguage } from '../utils/languages';
import { ScreenBackground, BackHeader } from '../components/PolyPuffUI';
import { useMascot, useAuth } from './_layout';
import { isSoundEnabled, setSoundEnabled, isHapticEnabled, setHapticEnabled } from '../services/sounds';
import { pushProfile } from '../services/profileService';
import { scaledFont } from '../utils/accessibility';
import { CUSTOMISE_PRACTICE_LIST_VISIBLE } from './customise';
import {
  getServerUrl, setServerUrl, checkHealth,
  DEFAULT_SERVER, LOCAL_SERVER, ANDROID_EMULATOR_SERVER,
} from '../services/api';
import { getReminderSettings, setReminderSettings, formatReminderTime } from '../services/notifications';
import { clearAuthSession, getSavedToken } from '../utils/authSession';
import { getSettingsTranslations } from '../utils/settingsTranslations';
import {
  isPlayBillingSupported, isPlayBillingConfigured, getPackages, purchasePackage, restorePurchases,
} from '../services/revenueCatService';
import type { PurchasesPackage } from 'react-native-purchases';
// ✅ NEW v6.3: Weekly digest settings card
import DigestSettings from '../components/DigestSettings';

type MascotContextValue = {
  mascotEnabled: boolean;
  setMascotEnabled: (value: boolean) => void;
};

const NATIVE_LANGUAGES = [
  'Afrikaans', 'Amharic', 'Arabic', 'Bengali', 'Bulgarian', 'Czech', 'Danish',
  'Dutch', 'Filipino', 'Finnish', 'French', 'German', 'Greek', 'Guarani', 'Gujarati',
  'Haitian Creole', 'Hausa', 'Hebrew', 'Hindi', 'Hungarian', 'Igbo', 'Indonesian',
  'Italian', 'Japanese', 'Korean', 'Malay', 'Mandarin', 'Marathi', 'Nepali',
  'Norwegian', 'Pashto', 'Persian', 'Polish', 'Portuguese', 'Punjabi', 'Quechua',
  'Romanian', 'Russian', 'Sinhala', 'Slovak', 'Spanish', 'Swahili', 'Swedish',
  'Tamil', 'Telugu', 'Thai', 'Turkish', 'Ukrainian', 'Urdu', 'Vietnamese',
  'Yoruba', 'Zulu',
];

const APP_LANGUAGES: Array<{ code: LangCode; label: string }> = [
  { code: 'en', label: 'English' }, { code: 'af', label: 'Afrikaans' },
  { code: 'am', label: 'Amharic' }, { code: 'ar', label: 'Arabic' },
  { code: 'bn', label: 'Bengali' }, { code: 'bg', label: 'Bulgarian' },
  { code: 'cs', label: 'Czech' }, { code: 'da', label: 'Danish' },
  { code: 'nl', label: 'Dutch' }, { code: 'fil', label: 'Filipino' },
  { code: 'fi', label: 'Finnish' }, { code: 'fr', label: 'French' },
  { code: 'de', label: 'German' }, { code: 'el', label: 'Greek' },
  { code: 'gn', label: 'Guarani' }, { code: 'gu', label: 'Gujarati' },
  { code: 'ht', label: 'Haitian Creole' }, { code: 'ha', label: 'Hausa' },
  { code: 'he', label: 'Hebrew' },
  { code: 'hi', label: 'Hindi' }, { code: 'hu', label: 'Hungarian' },
  { code: 'ig', label: 'Igbo' }, { code: 'id', label: 'Indonesian' },
  { code: 'it', label: 'Italian' }, { code: 'ja', label: 'Japanese' },
  { code: 'ko', label: 'Korean' }, { code: 'ms', label: 'Malay' },
  { code: 'zh', label: 'Mandarin' }, { code: 'mr', label: 'Marathi' },
  { code: 'ne', label: 'Nepali' }, { code: 'no', label: 'Norwegian' },
  { code: 'ps', label: 'Pashto' }, { code: 'fa', label: 'Persian' },
  { code: 'pl', label: 'Polish' }, { code: 'pt', label: 'Portuguese' },
  { code: 'pa', label: 'Punjabi' }, { code: 'qu', label: 'Quechua' },
  { code: 'ro', label: 'Romanian' }, { code: 'ru', label: 'Russian' },
  { code: 'si', label: 'Sinhala' }, { code: 'sk', label: 'Slovak' },
  { code: 'es', label: 'Spanish' }, { code: 'sw', label: 'Swahili' },
  { code: 'sv', label: 'Swedish' }, { code: 'ta', label: 'Tamil' },
  { code: 'te', label: 'Telugu' }, { code: 'th', label: 'Thai' },
  { code: 'tr', label: 'Turkish' }, { code: 'uk', label: 'Ukrainian' },
  { code: 'ur', label: 'Urdu' }, { code: 'vi', label: 'Vietnamese' },
  { code: 'yo', label: 'Yoruba' }, { code: 'zu', label: 'Zulu' },
];

const CEFR_LEVELS = [
  { code: 'A0', label: 'Pre-A1 starter', desc: 'No prior knowledge, basic words' },
  { code: 'A1', label: 'Beginner', desc: 'Basic words and phrases' },
  { code: 'A2', label: 'Elementary', desc: 'Simple sentences and familiar topics' },
  { code: 'B1', label: 'Intermediate', desc: 'Opinions, everyday problems, common tenses' },
  { code: 'B2', label: 'Upper-intermediate', desc: 'Complex sentences, nuance, idioms' },
  { code: 'C1', label: 'Advanced', desc: 'Flexible, sophisticated language' },
  { code: 'C2', label: 'Proficient', desc: 'Near-native precision' },
];

type PickerMode = 'nativeLanguage' | 'appLanguage' | 'cefrLevel' | null;

const PASSWORD_RESET_PROVIDERS = new Set(['email', 'password']);

// Free-tier monthly AI-request quota fallback - used only until the server's
// promptsLimit field is available (first render, or a cached pre-quota
// account blob). Must match FREE_MONTHLY_PROMPT_LIMIT in the backend's
// server.js; the server value always wins once a fresh account summary loads.
const FREE_MONTHLY_PROMPT_LIMIT_FALLBACK = 100;

export default function SettingsScreen() {
  const router = useRouter();
  const { colors: C } = useTheme();
  const { t: tRaw, wt, setLang, lang } = useLanguage();
  const settingsT = useMemo(() => getSettingsTranslations(lang), [lang]);
  const t = useMemo(() => ({ ...(tRaw as unknown as Record<string, string>), ...settingsT }), [tRaw, settingsT]);
  const isRtl = isRtlLanguage(lang);
  const rowDirection = isRtl ? 'row-reverse' : 'row';
  const textAlign = isRtl ? 'right' : 'left';
  const chevronStyle = isRtl ? { transform: [{ scaleX: -1 }] } : undefined;
  const ui = (key: string, fallback: string) => settingsT[key] ?? (tRaw as unknown as Record<string, string | undefined>)[key] ?? fallback;
  const tx = (key: string, params?: Record<string, string | number>) => {
    const value = ui(key, key);
    if (!params) return value;
    return value.replace(/\{(\w+)\}/g, (_, paramKey) => String(params[paramKey] ?? `{${paramKey}}`));
  };
  const brandName = 'Poly-Puff';
  const serverUrlPlaceholder = 'http://192.168.1.10:3000';
  const mascotCtx: MascotContextValue = (() => { try { return useMascot(); } catch { return { mascotEnabled: true, setMascotEnabled: (_value: boolean) => {} }; } })();
  const { resetAuth } = useAuth();
  const appVersion = Constants.expoConfig?.version || '1.0.0';

  const [ruleStats,       setRuleStats]       = useState({ totalRules: 340, categoryCount: 35, languageCount: 14 });
  const [soundOn,         setSoundOn]         = useState(isSoundEnabled());
  const [hapticOn,        setHapticOn]        = useState(isHapticEnabled());
  const [cacheSize,       setCacheSize]       = useState('...');
  const [reminderEnabled, setReminderEnabled] = useState(false);
  const [reminderHour,    setReminderHour]    = useState(18);
  const [reminderMinute,  setReminderMinute]  = useState(0);
  const [showTimePicker,  setShowTimePicker]  = useState(false);
  const [activeModuleCount, setActiveModuleCount] = useState(9);
  const [currentServerUrl, setCurrentServerUrl] = useState('');
  const [serverUrlInput, setServerUrlInput] = useState('');
  const [serverStatus, setServerStatus] = useState<string>('');
  const [backendVersion, setBackendVersion] = useState('-');
  const [accountEmail, setAccountEmail] = useState('');
  const [accountMethod, setAccountMethod] = useState('');
  const [accountToken, setAccountToken] = useState('');
  const [accountPlan, setAccountPlan] = useState<string>('');
  const [accountPlanTone, setAccountPlanTone] = useState<'free' | 'pro' | 'admin' | 'local'>('local');
  const [isRevenueCatBilling, setIsRevenueCatBilling] = useState(false);
  const [billingStatus, setBillingStatus] = useState('Not synced');
  const [billingDesc, setBillingDesc] = useState<string>('');
  const [promptsUsed, setPromptsUsed] = useState('-');
  const [promptDesc, setPromptDesc] = useState<string>('');
  const [promptUsage, setPromptUsage] = useState<{ used: number; limit: number | null; synced: boolean }>({ used: 0, limit: FREE_MONTHLY_PROMPT_LIMIT_FALLBACK, synced: false });
  const [emailReminderEnabled, setEmailReminderEnabled] = useState(false);
  const [emailReminderDesc, setEmailReminderDesc] = useState<string>('');
  const [emailReminderLoading, setEmailReminderLoading] = useState(false);
  const [loadingAccount, setLoadingAccount] = useState(false);
  const [resettingPassword, setResettingPassword] = useState(false);
  const [openingBilling, setOpeningBilling] = useState(false);
  const [nativeLanguage, setNativeLanguage] = useState('English');
  const [appLanguage, setAppLanguage] = useState<LangCode>('en');
  const [cefrLevel, setCefrLevel] = useState('B1');
  const [pickerMode, setPickerMode] = useState<PickerMode>(null);
  const normalizedAccountMethod = String(accountMethod || '').toLowerCase();
  const canChangePassword = !normalizedAccountMethod || PASSWORD_RESET_PROVIDERS.has(normalizedAccountMethod);
  const canUpgradePlan = accountPlanTone === 'free';
  const canOpenExternalPaymentLinks = Platform.OS !== 'android';
  const canShowPlanUpgrade = canUpgradePlan && canOpenExternalPaymentLinks;
  // Android can't open the external Paddle checkout (Play Store policy), so
  // it gets its own in-app Play Billing paywall instead - but only once
  // Play Billing is actually configured, so a build without the RevenueCat
  // API key set doesn't show an upgrade button that's guaranteed to fail.
  const canShowAndroidUpgrade = canUpgradePlan && isPlayBillingConfigured();
  // Android can't offer the external Paddle *checkout* (that would be a new
  // purchase steered outside Play Billing), but Google's payments policy
  // explicitly permits linking to an account-management page for a
  // subscription that already exists, even one purchased outside Google
  // Play - so an existing Paddle subscriber can still manage/cancel it from
  // here. Gated on already being Pro via something other than RevenueCat, so
  // this can never appear as a way to buy Pro for the first time on Android.
  const canManagePaddleBillingOnAndroid = Platform.OS === 'android' && accountPlanTone === 'pro' && !isRevenueCatBilling;
  const [paywallVisible, setPaywallVisible] = useState(false);
  const [paywallPackages, setPaywallPackages] = useState<PurchasesPackage[]>([]);
  const [paywallLoading, setPaywallLoading] = useState(false);
  const [purchasingId, setPurchasingId] = useState<string | null>(null);
  const promptBarPercent = promptUsage.limit !== null ? Math.min(100, Math.max(0, (promptUsage.used / promptUsage.limit) * 100)) : 100;
  const promptLimitReached = promptUsage.limit !== null && promptUsage.used >= promptUsage.limit;

  useEffect(() => {
    estimateCache();
    getServerUrl().then(url => {
      setCurrentServerUrl(url);
      setServerUrlInput(url);
    }).catch(() => {});
    // Stats: use hardcoded values as primary source.
    // Try to get live counts from backend; only update if the values are non-zero.
    getServerUrl().then(BASE =>
      fetch(`${BASE}/api/stats`)
        .then(r => r.json())
        .then(data => {
          const liveRules    = data.totalRules || 0;
          const liveCats     = data.categories ? Object.keys(data.categories).length : 0;
          const liveLangs    = data.languages  ? data.languages.length : 0;
          setRuleStats({
            totalRules:    liveRules > 0 ? liveRules : 340,
            categoryCount: liveCats  > 0 ? liveCats  : 35,
            languageCount: liveLangs > 0 ? liveLangs : 14,
          });
        })
        .catch(() => {}) // keep defaults on any network failure
    );
    getServerUrl()
      .then(() => checkHealth())
      .then(health => setBackendVersion(health?.version ? String(health.version) : '-'))
      .catch(() => setBackendVersion('-'));
    getReminderSettings().then(({ enabled, hour, minute }) => {
      setReminderEnabled(enabled);
      setReminderHour(hour);
      setReminderMinute(minute);
    }).catch(() => {});
  }, []);

  useFocusEffect(useCallback(() => {
    loadAccountSummary();
    loadLearningPrefs();
  }, []));

  const loadAccountSummary = async () => {
    try {
      setLoadingAccount(true);
      const [
        profileRaw, authIdentifier, authMethod, digestEmail,
        savedToken, cachedAccount,
      ] = await Promise.all([
        AsyncStorage.getItem('userProfile'),
        AsyncStorage.getItem('authIdentifier'),
        AsyncStorage.getItem('authMethod'),
        AsyncStorage.getItem('digestEmail'),
        getSavedWebToken(),
        AsyncStorage.getItem('accountSummary'),
      ]);

      let profileEmail = '';
      if (profileRaw) {
        const profile = JSON.parse(profileRaw);
        profileEmail = profile.email || profile.userEmail || profile.authIdentifier || '';
      }

      const email = [profileEmail, authIdentifier, digestEmail].find(v => v && v.includes('@')) || '';
      setAccountEmail(email);
      setAccountMethod(authMethod || (email ? 'email' : ''));

      if (cachedAccount) {
        applyAccountSummary(JSON.parse(cachedAccount));
      }

      if (savedToken) {
        setAccountToken(savedToken);
        loadEmailReminderPreference(savedToken);
        await refreshAccountSummary(savedToken);
      } else if (!cachedAccount) {
        setEmailReminderEnabled(false);
        setEmailReminderDesc(t.signInWebReminders);
        setLocalAccountSummary(email);
      }
    } catch {
      setAccountEmail('');
      setAccountMethod('');
      setEmailReminderEnabled(false);
      setEmailReminderDesc(t.signInWebReminders);
      setLocalAccountSummary('');
    } finally {
      setLoadingAccount(false);
    }
  };

  const normaliseCefrLevel = (level?: string) => {
    const clean = String(level || '').trim().toUpperCase();
    return CEFR_LEVELS.some(item => item.code === clean) ? clean : 'B1';
  };

  const getAppLanguageLabel = (code: LangCode) => APP_LANGUAGES.find(lang => lang.code === code)?.label || 'English';
  const getCefrLabel = (code: string) => {
    const level = CEFR_LEVELS.find(item => item.code === normaliseCefrLevel(code)) || CEFR_LEVELS[3];
    return `${level.code} - ${level.label}`;
  };

  const loadLearningPrefs = async () => {
    try {
      const raw = await AsyncStorage.getItem('userProfile');
      const profile = raw ? JSON.parse(raw) : {};
      const savedAppLanguage = APP_LANGUAGES.some(lang => lang.code === profile.appLanguage) ? profile.appLanguage : 'en';
      setNativeLanguage(profile.nativeLanguage || 'English');
      setAppLanguage(savedAppLanguage);
      setCefrLevel(normaliseCefrLevel(profile.level || profile.cefrLevel));
    } catch {
      setNativeLanguage('English');
      setAppLanguage('en');
      setCefrLevel('B1');
    }
  };

  const saveProfilePatch = async (patch: Record<string, unknown>) => {
    const raw = await AsyncStorage.getItem('userProfile');
    const profile = raw ? JSON.parse(raw) : {};
    const updated = { ...profile, ...patch };
    await AsyncStorage.setItem('userProfile', JSON.stringify(updated));
    pushProfile(updated).catch(() => {});
  };

  const saveNativeLanguage = async (value: string) => {
    setNativeLanguage(value);
    await saveProfilePatch({ nativeLanguage: value });
    setPickerMode(null);
  };

  const saveAppLanguage = async (value: LangCode) => {
    setAppLanguage(value);
    await setLang(value);
    await saveProfilePatch({ appLanguage: value });
    setPickerMode(null);
  };

  const saveCefrLevel = async (value: string) => {
    const level = normaliseCefrLevel(value);
    setCefrLevel(level);
    await saveProfilePatch({ level, cefrLevel: level });
    setPickerMode(null);
  };

  // Reload active module count whenever Settings gets focus
  useFocusEffect(
    React.useCallback(() => {
      AsyncStorage.getItem('practiceModuleConfig').then(raw => {
        if (raw) {
          try {
            const cfg = JSON.parse(raw);
            setActiveModuleCount(cfg.active ? cfg.active.length : 9);
          } catch {}
        }
      }).catch(() => {});
    }, [])
  );

  const handleSoundToggle  = async (val: boolean) => { setSoundOn(val);  await setSoundEnabled(val);  };
  const handleHapticToggle = async (val: boolean) => { setHapticOn(val); await setHapticEnabled(val); };

  const handleReminderToggle = async (val: boolean) => {
    setReminderEnabled(val);
    const saved = await setReminderSettings(val, reminderHour, reminderMinute);
    if (val && !saved) {
      setReminderEnabled(false);
      Alert.alert(
        t.notificationsDisabledTitle,
        t.notificationsEnableFailed
      );
    }
  };

  const handleTimeSelect = async (hour: number, minute: number) => {
    setReminderHour(hour);
    setReminderMinute(minute);
    setShowTimePicker(false);
    if (reminderEnabled) {
      const saved = await setReminderSettings(true, hour, minute);
      if (!saved) {
        setReminderEnabled(false);
        Alert.alert(
          t.notificationsDisabledTitle,
          t.notificationsUpdateFailed
        );
      }
    }
  };

  const testServerConnection = async () => {
    setServerStatus(t.serverTesting);
    try {
      const health = await checkHealth();
      const label = health?.rulesLoaded ? tx('serverConnectedRules', { rules: health.rulesLoaded }) : t.serverConnected;
      setServerStatus(label);
      Alert.alert(t.connectionOkTitle, t.serverHealthy);
      return true;
    } catch (e) {
      setServerStatus(t.serverNotReachable);
      Alert.alert(t.connectionFailedTitle, t.serverReachError);
      return false;
    }
  };

  const chooseServerUrl = async (url: string, label: string) => {
    await setServerUrl(url);
    setCurrentServerUrl(url);
    setServerUrlInput(url);
    setServerStatus('');
    Alert.alert(t.backendUpdatedTitle, tx('usingBackend', { label }));
  };

  const saveCustomServerUrl = async () => {
    const trimmed = serverUrlInput.trim().replace(/\/+$/, '');
    if (!trimmed.startsWith('http://') && !trimmed.startsWith('https://')) {
      Alert.alert(t.invalidUrlTitle, t.serverUrlInvalidMsg);
      return;
    }
    await setServerUrl(trimmed);
    setCurrentServerUrl(trimmed);
    setServerUrlInput(trimmed);
    setServerStatus('');
    await testServerConnection();
  };

  const getSavedWebToken = async () => getSavedToken();

  const formatBillingDate = (value?: string | null) => {
    if (!value) return '';
    const date = new Date(value);
    if (Number.isNaN(date.getTime())) return '';
    return date.toLocaleDateString(undefined, { year: 'numeric', month: 'long', day: 'numeric' });
  };

  const setLocalAccountSummary = (email: string) => {
    setAccountPlan(email ? t.notSyncedLower : t.localLabel);
    setAccountPlanTone('local');
    setBillingStatus('Not synced');
    setBillingDesc(email ? t.webAccountNotSyncedYet : t.noWebAccountSaved);
    setPromptsUsed('-');
    setPromptDesc(email ? t.promptUsageSyncsAfterSignIn : t.signInWebPromptUsage);
    setPromptUsage({ used: 0, limit: FREE_MONTHLY_PROMPT_LIMIT_FALLBACK, synced: false });
  };

  const applyAccountSummary = (account: any) => {
    if (account?.email) setAccountEmail(account.email);
    if (account?.provider) setAccountMethod(String(account.provider));

    if (account?.isAdmin) {
      setAccountPlan(t.adminPlan);
      setAccountPlanTone('admin');
    } else if (account?.isTeacher) {
      // No dedicated 'teacher' badge tone exists yet - reuse pro's styling,
      // the label text is what tells the two apart.
      setAccountPlan(ui('teacherPlanBadge', 'Teacher'));
      setAccountPlanTone('pro');
    } else if (account?.isPro) {
      setAccountPlan(t.proPlan);
      setAccountPlanTone('pro');
    } else {
      setAccountPlan(t.freePlan);
      setAccountPlanTone('free');
    }

    const limit = account?.promptsLimit !== undefined
      ? account.promptsLimit
      : (account?.isPro || account?.isAdmin ? null : FREE_MONTHLY_PROMPT_LIMIT_FALLBACK);
    const used = account?.isAdmin ? 0 : Number(account?.promptsUsed || 0);
    setPromptsUsed(limit !== null ? `${used} / ${limit}` : `${used}`);
    setPromptDesc(limit !== null ? tx('promptsRemainingToday', { count: Math.max(0, limit - used) }) : t.unlimitedPrompts);
    setPromptUsage({ used, limit, synced: true });

    const billing = account?.billing || {};
    const status = String(billing.paddleStatus || '').replace(/_/g, ' ');
    const periodEnd = formatBillingDate(billing.currentPeriodEnd);
    const cancelAt = formatBillingDate(billing.cancelAt);
    // Which portal to point "Manage Billing" at - Play Store only applies
    // when the active Pro entitlement actually came from RevenueCat.
    setIsRevenueCatBilling(billing.revenuecatActive === true);

    if (billing.revenuecatActive) {
      // Play Billing-sourced Pro - no Paddle record exists, so use the
      // RevenueCat expiry instead of the paddle_* fields above.
      const rcExpires = formatBillingDate(billing.revenuecatExpiresAt);
      setBillingStatus(t.activeBilling);
      setBillingDesc(rcExpires ? tx('renewsOn', { date: rcExpires }) : t.subscriptionActive);
    } else if (billing.paddleStatus === 'active') {
      setBillingStatus(t.activeBilling);
      setBillingDesc(periodEnd ? tx('renewsOn', { date: periodEnd }) : t.subscriptionActive);
    } else if (billing.paddleStatus === 'trialing') {
      setBillingStatus(t.trialBilling);
      setBillingDesc(periodEnd ? tx('trialEndsOn', { date: periodEnd }) : t.trialSubscriptionActive);
    } else if (billing.paddleStatus === 'past_due') {
      setBillingStatus(t.pastDue);
      setBillingDesc(t.paymentNeedsAttention);
    } else if (billing.paddleStatus === 'canceled' || billing.paddleStatus === 'cancelled') {
      setBillingStatus(t.canceledBilling);
      setBillingDesc(cancelAt ? tx('canceledOn', { date: cancelAt }) : t.subscriptionCanceled);
    } else if (account?.isPro || account?.isAdmin) {
      setBillingStatus(status || t.activeBilling);
      setBillingDesc(periodEnd ? tx('currentPeriodEndsOn', { date: periodEnd }) : t.billingDetailsSynced);
    } else {
      setBillingStatus(t.freePlan);
      setBillingDesc(t.noActiveSubscription);
    }
  };

  const refreshAccountSummary = async (token = accountToken) => {
    if (!token) {
      setLocalAccountSummary(accountEmail);
      return undefined;
    }

    setLoadingAccount(true);
    try {
      const BASE = await getServerUrl();
      const res = await fetch(`${BASE}/api/auth/me`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data.error || t.couldNotRefreshAccount);
      await AsyncStorage.setItem('accountSummary', JSON.stringify(data));
      applyAccountSummary(data);
      return data;
    } catch {
      setBillingDesc(t.couldNotRefreshAccount);
      return undefined;
    } finally {
      setLoadingAccount(false);
    }
  };

  const loadEmailReminderPreference = async (token = accountToken) => {
    if (!token) {
      setEmailReminderEnabled(false);
      setEmailReminderDesc(t.signInWebReminders);
      return;
    }

    setEmailReminderLoading(true);
    try {
      const BASE = await getServerUrl();
      const res = await fetch(`${BASE}/api/reminders/preferences`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data.error || t.couldNotLoadEmailReminders);

      setEmailReminderEnabled(!!data.enabled);
      setEmailReminderDesc(data.enabled
        ? t.emailReminderOnDesc
        : t.emailReminderOffDesc);
    } catch {
      setEmailReminderDesc(t.couldNotLoadEmailReminders);
    } finally {
      setEmailReminderLoading(false);
    }
  };

  const saveEmailReminderPreference = async (enabled: boolean) => {
    if (!accountToken) {
      setEmailReminderEnabled(false);
      Alert.alert(t.webSignInNeeded, t.signInWebReminders);
      return;
    }

    const previous = emailReminderEnabled;
    setEmailReminderEnabled(enabled);
    setEmailReminderLoading(true);
    try {
      const BASE = await getServerUrl();
      const res = await fetch(`${BASE}/api/reminders/preferences`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${accountToken}` },
        body: JSON.stringify({ enabled }),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data.error || t.couldNotSaveEmailReminder);

      setEmailReminderEnabled(!!data.enabled);
      setEmailReminderDesc(data.enabled
        ? t.emailReminderOnDesc
        : t.emailReminderOffDesc);
    } catch {
      setEmailReminderEnabled(previous);
      Alert.alert(t.emailLearningReminders, t.couldNotSaveEmailReminder);
    } finally {
      setEmailReminderLoading(false);
    }
  };

  const openBillingPortal = async () => {
    if (!accountToken) {
      Alert.alert(t.webSignInNeeded, t.billingManagementNeedsWebToken);
      return;
    }

    setOpeningBilling(true);
    try {
      const BASE = await getServerUrl();
      const res = await fetch(`${BASE}/api/paddle/portal-session`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${accountToken}` },
      });
      const data = await res.json().catch(() => ({}));

      if (res.ok && data.url) {
        await Linking.openURL(data.url);
      } else if (res.status === 404 && data.supportEmail) {
        await Linking.openURL('mailto:support@poly-puff.com?subject=Billing%20Help&body=Please%20help%20me%20manage%20my%20Poly-Puff%20billing.');
      } else {
        Alert.alert(t.billingPortalTitle, data.error || t.billingPortalCouldNotOpen);
      }
    } catch {
      Alert.alert(t.billingPortalTitle, t.billingPortalTryAgain);
    } finally {
      setOpeningBilling(false);
    }
  };

  const openPricing = () => {
    Linking.openURL('https://poly-puff.com/pricing');
  };

  const openAndroidPaywall = async () => {
    setPaywallVisible(true);
    setPaywallLoading(true);
    try {
      const pkgs = await getPackages();
      setPaywallPackages(pkgs);
    } finally {
      setPaywallLoading(false);
    }
  };

  // After a purchase/restore, RevenueCat's own SDK reflects the entitlement
  // instantly (used for the immediate UI update), but web_users.revenuecat_*
  // only updates once RevenueCat's webhook reaches the backend - usually a
  // few seconds, occasionally longer. Poll with backoff instead of a single
  // fixed-delay attempt, so a slow webhook doesn't leave stale billing status
  // on screen with no automatic follow-up.
  const syncAccountAfterEntitlementChange = () => {
    const delays = [2000, 4000, 8000];
    let attempt = 0;
    const tryRefresh = () => {
      setTimeout(async () => {
        const data = await refreshAccountSummary().catch(() => undefined);
        attempt += 1;
        if (data?.billing?.revenuecatActive || attempt >= delays.length) return;
        tryRefresh();
      }, delays[attempt]);
    };
    tryRefresh();
  };

  const handlePurchase = async (pkg: PurchasesPackage) => {
    setPurchasingId(pkg.identifier);
    try {
      const result = await purchasePackage(pkg);
      if (result.cancelled) return;
      if (!result.success) {
        Alert.alert(t.billingPortalTitle, result.error || t.billingPortalTryAgain);
        return;
      }
      setPaywallVisible(false);
      if (result.isTeacher) {
        setAccountPlan(ui('teacherPlanBadge', 'Teacher'));
        setAccountPlanTone('pro');
        Alert.alert(
          ui('teacherPlanBadge', 'Teacher'),
          ui('teacherPlanWelcome', 'Your Teacher plan is active. Use this app to check on your students\' progress, and the Poly-Puff website to create and assign lessons.'),
        );
      } else if (result.isPro) {
        setAccountPlan(t.proPlan);
        setAccountPlanTone('pro');
        Alert.alert(t.proPlan, t.subscriptionActive);
      } else {
        // Purchase completed and the user was charged, but the product isn't
        // (yet) linked to the "pro" entitlement in the RevenueCat dashboard -
        // don't leave them with zero feedback about a real transaction.
        Alert.alert(
          ui('purchaseProcessingTitle', 'Purchase Received'),
          ui('purchaseProcessingDesc', "Your purchase went through. It can take a minute to activate - if Pro access doesn't appear shortly, contact support with your purchase receipt."),
        );
      }
      syncAccountAfterEntitlementChange();
    } finally {
      setPurchasingId(null);
    }
  };

  const handleRestorePurchases = async () => {
    setPaywallLoading(true);
    try {
      const result = await restorePurchases();
      if (result.isTeacher) {
        setPaywallVisible(false);
        setAccountPlan(ui('teacherPlanBadge', 'Teacher'));
        setAccountPlanTone('pro');
        Alert.alert(ui('teacherPlanBadge', 'Teacher'), t.subscriptionActive);
        syncAccountAfterEntitlementChange();
      } else if (result.isPro) {
        setPaywallVisible(false);
        setAccountPlan(t.proPlan);
        setAccountPlanTone('pro');
        Alert.alert(t.proPlan, t.subscriptionActive);
        syncAccountAfterEntitlementChange();
      } else if (!result.success && result.error) {
        Alert.alert(t.billingPortalTitle, result.error);
      } else {
        Alert.alert(t.billingPortalTitle, t.noActiveSubscription);
      }
    } finally {
      setPaywallLoading(false);
    }
  };

  const signOut = () => {
    Alert.alert(t.signOut, t.signOutConfirmMsg, [
      { text: t.cancel, style: 'cancel' },
      { text: t.signOut, style: 'destructive', onPress: async () => {
        await clearAuthSession();
        await AsyncStorage.multiRemove(['webAuthToken', 'authToken', 'jwt', 'polyPuffToken']);
        setAccountToken('');
        setAccountEmail('');
        setAccountMethod('');
        setLocalAccountSummary('');
        setEmailReminderEnabled(false);
        setEmailReminderDesc(t.signInWebReminders);
        Alert.alert(t.signedOutTitle, t.signedOutMsg);
        resetAuth();
      }},
    ]);
  };

  const sendPasswordReset = async () => {
    if (!accountEmail) {
      Alert.alert(t.noEmailFoundTitle, t.noEmailFoundMsg);
      return;
    }

    setResettingPassword(true);
    try {
      const BASE = await getServerUrl();
      const res = await fetch(`${BASE}/api/auth/forgot-password`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: accountEmail }),
      });
      const data = await res.json();

      if (res.ok) {
        Alert.alert(t.resetLinkSentTitle, tx('resetLinkSentMsg', { email: accountEmail }));
      } else {
        Alert.alert(t.couldNotSendResetTitle, data.error || t.tryAgainMoment);
      }
    } catch {
      Alert.alert(t.connectionErrorTitle, t.internetTryAgain);
    } finally {
      setResettingPassword(false);
    }
  };

  const TIME_OPTIONS: Array<{ hour: number; minute: number }> = [];
  for (let h = 6; h <= 22; h++) {
    TIME_OPTIONS.push({ hour: h, minute: 0 });
    TIME_OPTIONS.push({ hour: h, minute: 30 });
  }

  const estimateCache = async () => {
    try {
      const keys = ['rulesCache_all_all', 'exerciseCache'];
      let total = 0;
      for (const key of keys) { const val = await AsyncStorage.getItem(key); if (val) total += val.length; }
      if (total > 1000000) setCacheSize(`${(total / 1048576).toFixed(1)} MB`);
      else if (total > 1000) setCacheSize(`${(total / 1024).toFixed(0)} KB`);
      else setCacheSize(`${total} bytes`);
    } catch (e) { setCacheSize(t.unknownSize); }
  };

  const clearCache = () => {
    Alert.alert(t.clearCache, t.clearCacheConfirmDesc, [
      { text: t.cancel, style: 'cancel' },
      { text: t.clearAction, style: 'destructive', onPress: async () => { await AsyncStorage.multiRemove(['rulesCache_all_all', 'exerciseCache']); setCacheSize('0 bytes'); }},
    ]);
  };

  const clearAllProgress = () => {
    Alert.alert(t.clearAllProgressLabel, t.clearAllProgressConfirmDesc, [
      { text: t.cancel, style: 'cancel' },
      { text: t.deleteEverything, style: 'destructive', onPress: async () => {
        await AsyncStorage.multiRemove(['mistakeHistory', 'totalXP', 'sessionHistory', 'completedChallenges']);
        Alert.alert(t.doneTitle, t.allProgressCleared);
      }},
    ]);
  };

  const resetOnboarding = async () => {
    await AsyncStorage.removeItem('onboardingComplete');
    Alert.alert(t.onboardingResetTitle, t.onboardingResetMsg);
  };

  const resetDefaults = () => {
    Alert.alert(
      (t as any).setToDefaultQ,
      t.resetDefaultsConfirmDesc,
      [
        { text: t.cancel, style: 'cancel' },
        { text: t.resetAction, style: 'destructive', onPress: async () => {
          await AsyncStorage.multiRemove([
            'soundEnabled', 'hapticEnabled', 'mascotEnabled',
            'streakReminderEnabled', 'streakReminderHour', 'streakReminderMinute',
            'practiceModuleConfig',
          ]);

          const saved = await AsyncStorage.getItem('userProfile');
          if (saved) {
            const profile = JSON.parse(saved);
            delete profile.nativeLanguage;
            delete profile.level;
            delete profile.appLanguage;
            delete profile.cefrLevel;
            await AsyncStorage.setItem('userProfile', JSON.stringify(profile));
          }

          setSoundOn(true);
          setHapticOn(true);
          setReminderEnabled(false);
          setReminderHour(18);
          setReminderMinute(0);
          setActiveModuleCount(9);
          setNativeLanguage('English');
          setAppLanguage('en');
          setCefrLevel('B1');
          await setLang('en');
          mascotCtx.setMascotEnabled(true);

          Alert.alert(t.defaultsRestoredTitle, t.defaultsRestoredMsg);
        }},
      ]
    );
  };

  const clearProfile = () => {
    Alert.alert(t.clearProfileInfo, t.clearProfileConfirmDesc, [
      { text: t.cancel, style: 'cancel' },
      { text: t.clearProfileAction, style: 'destructive', onPress: async () => {
        const saved = await AsyncStorage.getItem('userProfile');
        if (saved) { const p = JSON.parse(saved); await AsyncStorage.setItem('userProfile', JSON.stringify({ nativeLanguage: p.nativeLanguage, level: p.level })); }
        await AsyncStorage.removeItem('profilePic');
        Alert.alert(t.doneTitle, t.profileClearedMsg);
      }},
    ]);
  };

  type SectionProps = {
    title: string;
    icon: ReactNode;
    children: ReactNode;
  };

  type RowProps = {
    label: string;
    desc?: string;
    right?: ReactNode;
    onPress?: (event: GestureResponderEvent) => void | Promise<unknown>;
    danger?: boolean;
    accessibilityLabel?: string;
    accessibilityHint?: string;
  };

  const Section = ({ title, icon, children }: SectionProps) => (
    <View style={{ backgroundColor: C.card, borderRadius: 16, marginBottom: 12, borderWidth: 1, borderColor: C.border + '15', overflow: 'hidden' }}>
      <View style={{ flexDirection: rowDirection, alignItems: 'center', gap: 10, padding: 16, paddingBottom: 8 }}>
        {icon}
        <Text style={{ flex: 1, fontSize: scaledFont(15), fontWeight: '700', color: C.text, textAlign }} accessibilityRole="header">{title}</Text>
      </View>
      {children}
    </View>
  );

  const Row = ({ label, desc, right, onPress, danger, accessibilityLabel: a11yLabel, accessibilityHint: a11yHint }: RowProps) => (
    <TouchableOpacity
      style={{ flexDirection: rowDirection, alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingVertical: 14, borderTopWidth: 1, borderTopColor: C.border + '10', minHeight: 44 }}
      onPress={onPress}
      disabled={!onPress}
      activeOpacity={onPress ? 0.6 : 1}
      accessibilityRole={onPress ? 'button' : 'text'}
      accessibilityLabel={a11yLabel || label}
      accessibilityHint={a11yHint}
    >
      <View style={{ flex: 1, marginRight: isRtl ? 0 : 12, marginLeft: isRtl ? 12 : 0 }}>
        <Text style={{ fontSize: scaledFont(15), color: danger ? C.red : C.text, textAlign }}>{label}</Text>
        {desc && <Text style={{ fontSize: scaledFont(12), color: C.textMuted, marginTop: 2, textAlign }}>{desc}</Text>}
      </View>
      {right}
    </TouchableOpacity>
  );

  const PlanBadge = ({ label, tone }: { label: string; tone: 'free' | 'pro' | 'admin' | 'local' }) => {
    const palette = {
      free: { bg: (C.cardAlt || C.card) + '80', fg: C.textMuted, border: C.border + '40' },
      pro: { bg: (C.emerald || '#00E5A0') + '20', fg: C.emerald || '#00E5A0', border: (C.emerald || '#00E5A0') + '40' },
      admin: { bg: (C.purple || '#B06CFF') + '20', fg: C.purple || '#B06CFF', border: (C.purple || '#B06CFF') + '40' },
      local: { bg: (C.cyan || '#00E5FF') + '12', fg: C.textMuted, border: C.border + '30' },
    }[tone];

    return (
      <View style={{ backgroundColor: palette.bg, borderColor: palette.border, borderWidth: 1, paddingHorizontal: 10, paddingVertical: 4, borderRadius: 999 }}>
        <Text style={{ fontSize: scaledFont(11), fontWeight: '800', color: palette.fg }}>{label}</Text>
      </View>
    );
  };

  const renderPreferencePicker = () => {
    if (!pickerMode) return null;

    const title = pickerMode === 'nativeLanguage'
      ? t.nativeLanguage
      : pickerMode === 'appLanguage'
        ? t.appLanguage
        : `CEFR ${t.level}`;
    const options = pickerMode === 'nativeLanguage'
      ? NATIVE_LANGUAGES.map(name => ({ key: name, label: name, desc: t.nativeLangDesc, flag: flagForEnglishName(name) }))
      : pickerMode === 'appLanguage'
        ? APP_LANGUAGES.map(lang => ({ key: lang.code, label: lang.label, desc: lang.code.toUpperCase(), flag: flagForLangCode(lang.code) }))
        : CEFR_LEVELS.map(level => ({ key: level.code, label: `${level.code} - ${level.label}`, desc: level.desc, flag: '' }));
    const selected = pickerMode === 'nativeLanguage' ? nativeLanguage : pickerMode === 'appLanguage' ? appLanguage : cefrLevel;

    const handleSelect = async (key: string) => {
      if (pickerMode === 'nativeLanguage') await saveNativeLanguage(key);
      else if (pickerMode === 'appLanguage') await saveAppLanguage(key as LangCode);
      else await saveCefrLevel(key);
    };

    return (
      <Modal visible transparent animationType="fade" onRequestClose={() => setPickerMode(null)}>
        <TouchableOpacity style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'flex-end' }} activeOpacity={1} onPress={() => setPickerMode(null)}>
          <View style={{ backgroundColor: C.card, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 20, paddingBottom: 36, maxHeight: '72%' }}
            accessibilityViewIsModal={true}>
            <Text style={{ fontSize: scaledFont(18), fontWeight: '800', color: C.text, marginBottom: 14, textAlign: 'center' }}
              accessibilityRole="header">{title}</Text>
            <ScrollView showsVerticalScrollIndicator={false}>
              {options.map(option => {
                const isSelected = selected === option.key;
                return (
                  <TouchableOpacity
                    key={option.key}
                    style={{
                      flexDirection: rowDirection, alignItems: 'center', gap: 12,
                      paddingVertical: 13, paddingHorizontal: 14, borderRadius: 10, marginBottom: 4,
                      backgroundColor: isSelected ? (C.cyan || '#00E5FF') + '18' : 'transparent',
                      borderWidth: isSelected ? 1 : 0, borderColor: (C.cyan || '#00E5FF') + '40',
                      minHeight: 44,
                    }}
                    onPress={() => handleSelect(option.key)}
                    accessibilityRole="button"
                    accessibilityLabel={tx('selectOption', { label: option.label })}
                    accessibilityState={{ selected: isSelected }}
                  >
                    {option.flag ? (
                      <Text
                        style={{ fontSize: scaledFont(22), lineHeight: scaledFont(26) }}
                        accessibilityElementsHidden
                        importantForAccessibility="no"
                      >
                        {option.flag}
                      </Text>
                    ) : null}
                    <View style={{ flex: 1 }}>
                      <Text style={{ fontSize: scaledFont(15), fontWeight: isSelected ? '800' : '600', color: isSelected ? (C.cyan || '#00E5FF') : C.text, textAlign }}>
                        {option.label}
                      </Text>
                      <Text style={{ fontSize: scaledFont(12), color: C.textMuted, marginTop: 2, textAlign }}>{option.desc}</Text>
                    </View>
                    {isSelected && <CheckCircle size={17} color={C.cyan || '#00E5FF'} />}
                  </TouchableOpacity>
                );
              })}
            </ScrollView>
            <TouchableOpacity
              style={{ paddingVertical: 12, alignItems: 'center', marginTop: 8, minHeight: 44 }}
              onPress={() => setPickerMode(null)}
              accessibilityRole="button"
              accessibilityLabel={t.cancel}
            >
              <Text style={{ fontSize: scaledFont(14), color: C.textMuted }}>{t.cancel}</Text>
            </TouchableOpacity>
          </View>
        </TouchableOpacity>
      </Modal>
    );
  };

  return (
    <ScreenBackground style={{ flex: 1 }}>
      <BackHeader title={t.settingsTitle} subtitle={undefined} onPress={undefined} rightElement={undefined} style={undefined} />
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 40 }}>
        <Image
          source={{ uri: 'data:image/png;base64,' }}
          style={{ height: 36, width: 180, resizeMode: 'contain', marginTop: 10 }}
          accessibilityLabel={brandName}
        />
        <Text style={{ fontSize: scaledFont(13), color: C.textSec, marginTop: 2, marginBottom: 20, textAlign }}>{t.settingsScreenDesc}</Text>

        <Section title={t.accountSection} icon={<User size={18} color={C.cyan || '#00E5FF'} />}>
          <Row
            label={t.emailLabel}
            desc={accountEmail || t.noEmailSaved}
            right={<Text style={{ fontSize: scaledFont(13), color: C.textMuted }}>{accountMethod || t.localLabel}</Text>}
            accessibilityLabel={`Account email: ${accountEmail || 'not saved'}`}
          />
          <Row
            label={t.planLabel}
            desc={loadingAccount ? t.refreshingAccountDetails : undefined}
            right={
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                <PlanBadge label={accountPlan} tone={accountPlanTone} />
                {(canShowPlanUpgrade || canShowAndroidUpgrade) && (
                  <TouchableOpacity
                    onPress={canShowAndroidUpgrade ? openAndroidPaywall : openPricing}
                    style={{
                      backgroundColor: C.cyan || '#00E5FF',
                      paddingHorizontal: 10,
                      paddingVertical: 5,
                      borderRadius: 8,
                      minHeight: 32,
                      justifyContent: 'center',
                    }}
                    accessibilityRole="button"
                    accessibilityLabel={t.viewPricing}
                  >
                    <Text style={{ fontSize: scaledFont(11), fontWeight: '800', color: '#06111F' }}>{t.viewPricing}</Text>
                  </TouchableOpacity>
                )}
              </View>
            }
            accessibilityLabel={tx('accountPlanA11y', { plan: accountPlan })}
          />
          <Row
            label={t.billingStatus}
            desc={billingDesc}
            right={<Text style={{ fontSize: scaledFont(13), color: C.textMuted, textTransform: 'capitalize' }}>{billingStatus}</Text>}
            accessibilityLabel={tx('billingStatusA11y', { status: billingStatus })}
          />
          <Row
            label={t.promptsUsedToday}
            desc={promptDesc}
            right={<Text style={{ fontSize: scaledFont(13), color: C.textMuted }}>{promptsUsed}</Text>}
            accessibilityLabel={tx('promptsUsedA11y', { count: promptsUsed })}
          />
          {promptUsage.synced && (
            <View
              style={{
                paddingHorizontal: 16,
                paddingBottom: 14,
                borderTopWidth: 1,
                borderTopColor: C.border + '10',
              }}
              accessibilityLabel={promptUsage.limit
                ? `Prompt usage ${Math.round(promptBarPercent)} percent`
                : 'Prompt usage unlimited'}
            >
              <View style={{ height: 6, borderRadius: 999, overflow: 'hidden', backgroundColor: C.border + '25' }}>
                <View
                  style={{
                    height: '100%',
                    width: `${promptBarPercent}%`,
                    backgroundColor: promptLimitReached ? C.red || '#FF4D6A' : C.cyan || '#00E5FF',
                  }}
                />
              </View>
            </View>
          )}
          <Row
            label={ui('emailLearningReminders', 'Email Learning Reminders')}
            desc={emailReminderLoading ? 'Loading email reminder preference...' : emailReminderDesc}
            right={
              <Switch
                value={emailReminderEnabled}
                onValueChange={saveEmailReminderPreference}
                disabled={!accountToken || emailReminderLoading}
                trackColor={{ false: '#D1D5DB', true: (C.cyan || '#00E5FF') + '50' }}
                thumbColor={emailReminderEnabled ? C.cyan || '#00E5FF' : C.textMuted}
                accessibilityRole="switch"
                accessibilityLabel={ui('emailLearningReminders', 'Email learning reminders')}
                accessibilityState={{ checked: emailReminderEnabled, disabled: !accountToken || emailReminderLoading }}
              />
            }
            accessibilityLabel={`${ui('emailLearningReminders', 'Email learning reminders')} ${emailReminderEnabled ? 'on' : 'off'}`}
          />
          <Row
            label={t.refreshAccount}
            desc={t.refreshAccountDesc}
            onPress={async () => {
              await refreshAccountSummary();
              await loadEmailReminderPreference();
            }}
            right={<RefreshCw size={16} color={C.textMuted} />}
            accessibilityLabel={ui('refreshAccountDetails', 'Refresh account details')}
          />
          {canChangePassword && (
            <Row
              label={t.changePasswordLabel}
              desc={t.changePasswordDesc}
              onPress={sendPasswordReset}
              right={resettingPassword
                ? <Text style={{ fontSize: scaledFont(13), color: C.textMuted }}>{ui('sending', 'Sending...')}</Text>
                : <KeyRound size={16} color={accountEmail ? C.textMuted : C.border || C.textMuted} />}
              accessibilityLabel={ui('sendPasswordReset', 'Send a password reset email')}
              accessibilityHint={t.changePasswordDesc}
            />
          )}
          <Row
            label={t.signOut}
            desc={t.signOutDesc}
            onPress={signOut}
            right={<LogOut size={16} color={C.red || '#FF4D6A'} />}
            danger
            accessibilityLabel={ui('signOutDevice', 'Sign out of this device')}
          />
        </Section>

        {canOpenExternalPaymentLinks && (
          <Section title={t.billingSection} icon={<CreditCard size={18} color={C.emerald || '#00E5A0'} />}>
            <Row
              label={t.manageBilling}
              desc={t.manageBillingDesc}
              onPress={openBillingPortal}
              right={openingBilling
                  ? <Text style={{ fontSize: scaledFont(13), color: C.textMuted }}>{ui('opening', 'Opening...')}</Text>
                : <ExternalLink size={16} color={C.textMuted} />}
              accessibilityLabel={ui('openBillingPortal', 'Open billing portal')}
            />
            <Row
              label="7-Day Money-Back Guarantee"
              desc={t.refundPolicyDesc}
              onPress={() => Linking.openURL('https://poly-puff.com/refund')}
              right={<ExternalLink size={16} color={C.textMuted} />}
              accessibilityLabel={ui('moneyBackGuarantee', 'View 7-day money-back guarantee. Opens external link.')}
            />
          </Section>
        )}

        {isPlayBillingSupported() && isRevenueCatBilling && (
          <Section title={t.billingSection} icon={<CreditCard size={18} color={C.emerald || '#00E5A0'} />}>
            <Row
              label={t.manageBilling}
              desc={ui('manageBillingDescGoogle', 'Open Google Play to manage your subscription, payment method, or cancellation')}
              onPress={() => Linking.openURL(`https://play.google.com/store/account/subscriptions?package=${Constants.expoConfig?.android?.package || 'com.polypuff.app'}`)}
              right={<ExternalLink size={16} color={C.textMuted} />}
              accessibilityLabel={ui('openBillingPortal', 'Manage subscription in Google Play')}
            />
            <Row
              label={ui('restorePurchases', 'Restore Purchases')}
              desc={ui('restorePurchasesDesc', 'Already subscribed on this Google account? Restore access.')}
              onPress={handleRestorePurchases}
              right={<RefreshCw size={16} color={C.textMuted} />}
              accessibilityLabel={ui('restorePurchases', 'Restore purchases')}
            />
          </Section>
        )}

        {canManagePaddleBillingOnAndroid && (
          <Section title={t.billingSection} icon={<CreditCard size={18} color={C.emerald || '#00E5A0'} />}>
            <Row
              label={t.manageBilling}
              desc={t.manageBillingDesc}
              onPress={openBillingPortal}
              right={openingBilling
                  ? <Text style={{ fontSize: scaledFont(13), color: C.textMuted }}>{ui('opening', 'Opening...')}</Text>
                : <ExternalLink size={16} color={C.textMuted} />}
              accessibilityLabel={ui('openBillingPortal', 'Open billing portal')}
            />
          </Section>
        )}

        <Section title={`${t.languagesSection} & ${t.level}`} icon={<Globe size={18} color={C.cyan || '#00E5FF'} />}>
          <Row
            label={t.nativeLanguage}
            desc={t.nativeLangDesc}
            onPress={() => setPickerMode('nativeLanguage')}
            right={<Text style={{ fontSize: scaledFont(13), color: C.textMuted }}>{nativeLanguage}</Text>}
            accessibilityLabel={`${t.nativeLanguage}: ${nativeLanguage}`}
            accessibilityHint={t.nativeLangDesc}
          />
          <Row
            label={t.appLanguage}
            desc={t.appLangDesc}
            onPress={() => setPickerMode('appLanguage')}
            right={
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                <Languages size={14} color={C.textMuted} />
                <Text style={{ fontSize: scaledFont(13), color: C.textMuted }}>{getAppLanguageLabel(appLanguage)}</Text>
              </View>
            }
            accessibilityLabel={`${t.appLanguage}: ${getAppLanguageLabel(appLanguage)}`}
            accessibilityHint={t.appLangDesc}
          />
          <Row
            label={`CEFR ${t.level}`}
            desc={t.cefrLevelDesc}
            onPress={() => setPickerMode('cefrLevel')}
            right={
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                <BookOpen size={14} color={C.textMuted} />
                <Text style={{ fontSize: scaledFont(13), color: C.textMuted }}>{getCefrLabel(cefrLevel)}</Text>
              </View>
            }
            accessibilityLabel={`CEFR ${t.level}: ${getCefrLabel(cefrLevel)}`}
            accessibilityHint={t.cefrLevelDesc}
          />
        </Section>

        {/* ── CUSTOMISE PRACTICE ──────────────────────────────────────── */}
        {CUSTOMISE_PRACTICE_LIST_VISIBLE && (
        <Section title={t.practice} icon={<Pencil size={18} color={C.pink || '#FF6EB4'} />}>
          <Row
            label={t.customisePracticeList}
            desc={t.customPracticeDesc}
            onPress={() => router.push('/customise')}
            right={
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                <View style={{ backgroundColor: (C.emerald || '#00E5A0') + '20', paddingHorizontal: 8, paddingVertical: 3, borderRadius: 8 }}>
                  <Text style={{ fontSize: scaledFont(11), fontWeight: '700', color: C.emerald || '#00E5A0' }}>
                    {tx('activeCount', { count: activeModuleCount })}
                  </Text>
                </View>
                <ChevronRight size={14} color={C.textMuted} style={chevronStyle} />
              </View>
            }
            accessibilityLabel={ui('customisePracticeList', 'Customise your practice list - choose exercises and reorder them')}
            accessibilityHint={t.customPracticeDesc}
          />
        </Section>
        )}

        {/* Backend section - dev-only. Hidden in production builds so end users
            can't accidentally repoint the app at a broken or hostile URL.
            Visible in Expo Go and development builds for debugging. */}
        {__DEV__ && (
        <Section title={t.backend} icon={<Server size={18} color={C.cyan || '#00E5FF'} />}>
          <Row
            label={t.currentServer}
            desc={currentServerUrl || t.loadingEllipsis}
            right={<Text style={{ fontSize: scaledFont(12), color: C.textMuted }}>{serverStatus || t.notTested}</Text>}
            accessibilityLabel={tx('currentBackendServerA11y', { server: currentServerUrl || t.loading.toLowerCase() })}
          />
          <View style={{ paddingHorizontal: 16, paddingVertical: 12, borderTopWidth: 1, borderTopColor: C.border + '10' }}>
            <TextInput
              style={{
                backgroundColor: C.inputBg || C.bg,
                borderRadius: 10,
                padding: 12,
                fontSize: scaledFont(14),
                color: C.text,
                borderWidth: 1,
                borderColor: C.border + '40',
                marginBottom: 8,
                textAlign,
              }}
              value={serverUrlInput}
              onChangeText={setServerUrlInput}
              placeholder={serverUrlPlaceholder}
              placeholderTextColor={C.textMuted}
              autoCapitalize="none"
              autoCorrect={false}
              keyboardType="url"
              accessibilityLabel={ui('customBackendServerUrl', 'Custom backend server URL')}
            />
            <TouchableOpacity
              style={{ backgroundColor: C.cyan || '#00E5FF', borderRadius: 10, paddingVertical: 11, alignItems: 'center', minHeight: 44 }}
              onPress={saveCustomServerUrl}
              accessibilityRole="button"
              accessibilityLabel={ui('saveTestBackendUrl', 'Save and test custom backend URL')}
            >
              <Text style={{ fontSize: scaledFont(14), fontWeight: '700', color: '#07111F' }}>{t.saveAndTest}</Text>
            </TouchableOpacity>
          </View>
          <Row
            label={t.useLocalDesktop}
            desc={LOCAL_SERVER}
            onPress={() => chooseServerUrl(LOCAL_SERVER, t.localDesktopBackend)}
            right={<ChevronRight size={16} color={C.textMuted} style={chevronStyle} />}
            accessibilityLabel={ui('useLocalBackend', 'Use local desktop backend')}
          />
          <Row
            label={t.useAndroidEmulator}
            desc={ANDROID_EMULATOR_SERVER}
            onPress={() => chooseServerUrl(ANDROID_EMULATOR_SERVER, t.androidEmulatorBackend)}
            right={<ChevronRight size={16} color={C.textMuted} style={chevronStyle} />}
            accessibilityLabel={ui('useEmulatorBackend', 'Use Android emulator backend')}
          />
          <Row
            label={t.useProduction}
            desc={DEFAULT_SERVER}
            onPress={() => chooseServerUrl(DEFAULT_SERVER, t.productionBackend)}
            right={<ChevronRight size={16} color={C.textMuted} style={chevronStyle} />}
            accessibilityLabel={ui('useProductionBackend', 'Use production backend')}
          />
          <Row
            label={t.testConnection}
            desc={t.checksApiHealth}
            onPress={testServerConnection}
            right={<ChevronRight size={16} color={C.textMuted} style={chevronStyle} />}
            accessibilityLabel={ui('testBackendConnection', 'Test backend connection')}
          />
        </Section>
        )}

        {/* ── POLY-PUFF MASCOT ─────────────────────────────────────────── */}
        <Section title={`${brandName} ${t.mascot}`} icon={<Image source={require('../assets/polypuff-transparent.png')} style={{ width: 22, height: 22 }} resizeMode="contain" />}>
          <Row
            label={t.showMascot}
            desc={t.showMascotDesc}
            right={
              <Switch
                value={mascotCtx.mascotEnabled}
                onValueChange={mascotCtx.setMascotEnabled}
                trackColor={{ false: '#D1D5DB', true: (C.cyan || C.emerald) + '50' }}
                thumbColor={mascotCtx.mascotEnabled ? C.cyan || C.emerald : C.textMuted}
                accessibilityRole="switch"
                accessibilityLabel={ui('showMascot', 'Show Poly-Puff mascot')}
                accessibilityState={{ checked: mascotCtx.mascotEnabled }}
              />
            }
          />
        </Section>

        {/* ── SOUND & HAPTICS ──────────────────────────────────────────── */}
        <Section title={t.soundHapticsSection} icon={<Volume2 size={18} color={C.emerald} />}>
          <Row
            label={t.soundEffects}
            desc={t.soundEffectsDesc}
            right={
              <Switch
                value={soundOn}
                onValueChange={handleSoundToggle}
                trackColor={{ false: '#D1D5DB', true: C.emerald + '50' }}
                thumbColor={soundOn ? C.emerald : C.textMuted}
                accessibilityRole="switch"
                accessibilityLabel={ui('soundEffects', 'Sound effects')}
                accessibilityState={{ checked: soundOn }}
              />
            }
          />
          <Row
            label={t.hapticFeedback}
            desc={t.hapticFeedbackDesc}
            right={
              <Switch
                value={hapticOn}
                onValueChange={handleHapticToggle}
                trackColor={{ false: '#D1D5DB', true: C.emerald + '50' }}
                thumbColor={hapticOn ? C.emerald : C.textMuted}
                accessibilityRole="switch"
                accessibilityLabel={ui('hapticFeedback', 'Haptic feedback')}
                accessibilityState={{ checked: hapticOn }}
              />
            }
          />
        </Section>

        {/* ── STREAK REMINDERS ─────────────────────────────────────────── */}
        <Section title={ui('streakReminders', 'Notifications & Streak Reminders')} icon={<Bell size={18} color={C.purple || '#B06CFF'} />}>
          <Row
            label={ui('dailyReminder', 'Daily Reminder')}
            desc={t.keepStreakAliveDesc}
            right={
              <Switch
                value={reminderEnabled}
                onValueChange={handleReminderToggle}
                trackColor={{ false: '#D1D5DB', true: (C.purple || '#B06CFF') + '50' }}
                thumbColor={reminderEnabled ? C.purple || '#B06CFF' : C.textMuted}
                accessibilityRole="switch"
                accessibilityLabel={ui('dailyStreakReminder', 'Daily streak reminder')}
                accessibilityState={{ checked: reminderEnabled }}
              />
            }
          />
          {reminderEnabled && (
            <Row
              label={ui('reminderTime', 'Reminder Time')}
              desc={tx('reminderAtDesc', { time: formatReminderTime(reminderHour, reminderMinute) })}
              onPress={() => setShowTimePicker(true)}
              right={
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                  <Text style={{ fontSize: scaledFont(14), fontWeight: '700', color: C.purple || '#B06CFF' }}>
                    {formatReminderTime(reminderHour, reminderMinute)}
                  </Text>
                  <ChevronRight size={14} color={C.textMuted} style={chevronStyle} />
                </View>
              }
              accessibilityLabel={tx('reminderAtDesc', { time: formatReminderTime(reminderHour, reminderMinute) })}
              accessibilityHint={ui('chooseReminderTime', 'Choose Reminder Time')}
            />
          )}
        </Section>

        {/* ── WEEKLY DIGEST ────────────────────────────────────────────── */}
        {/* ✅ NEW v6.3: Self-contained digest settings card.               */}
        {/* Handles toggle, email input, send-now, and last-sent date.      */}
        <View style={{ marginBottom: 12 }}>
          <View style={{ flexDirection: rowDirection, alignItems: 'center', gap: 8, paddingHorizontal: 4, marginBottom: 6 }}>
            <Mail size={15} color={C.cyan || '#00D9FF'} />
            <Text style={{ fontSize: scaledFont(13), fontWeight: '700', color: C.textMuted, letterSpacing: 0.5 }} accessibilityRole="header">
              {t.weeklyDigestSection}
            </Text>
          </View>
          <DigestSettings />
        </View>

        {/* ── OFFLINE CACHE ────────────────────────────────────────────── */}
        <Section title={t.offlineCacheSection} icon={<Download size={18} color={C.amber} />}>
          <Row label={t.cachedData} desc={t.cachedDataDesc.replace('{size}', cacheSize)}
            right={<Text style={{ fontSize: scaledFont(13), color: C.textMuted }}>{cacheSize}</Text>}
            accessibilityLabel={`${t.cachedData}: ${cacheSize}`} />
          <Row label={t.clearCache} desc={t.clearCacheDesc}
            onPress={clearCache} right={<Trash2 size={16} color={C.textMuted} />}
            accessibilityLabel={ui('clearOfflineCache', 'Clear offline cache')}
            accessibilityHint={t.clearCacheConfirmDesc} />
        </Section>

        {/* ── DATA MANAGEMENT ──────────────────────────────────────────── */}
        <Section title={t.dataManagementSection} icon={<Shield size={18} color={C.red} />}>
          <Row label={t.setToDefault} desc={t.setToDefaultDesc}
            onPress={resetDefaults} right={<RotateCcw size={16} color={C.red} />} danger
            accessibilityLabel={ui('resetDefaults', 'Set settings to default')}
            accessibilityHint={t.setToDefaultDesc} />
          <Row label={t.resetOnboardingLabel} desc={t.resetOnboardingDesc}
            onPress={resetOnboarding} right={<RotateCcw size={16} color={C.textMuted} />}
            accessibilityLabel={ui('resetOnboarding', 'Reset onboarding walkthrough')} />
          <Row label={t.clearProfileInfo} desc={t.clearProfileInfoDesc}
            onPress={clearProfile} right={<ChevronRight size={16} color={C.textMuted} style={chevronStyle} />}
            accessibilityLabel={t.clearProfileInfo} />
          <Row label={t.clearAllProgressLabel} desc={t.clearAllProgressDesc}
            onPress={clearAllProgress} right={<Trash2 size={16} color={C.red} />} danger
            accessibilityLabel={ui('clearAllProgressPermanently', 'Clear all progress permanently. This cannot be undone.')}
            accessibilityHint={t.clearAllProgressDesc} />
        </Section>

        <Section title={ui('communitySection', 'Community')} icon={<Trophy size={18} color={C.cyan || '#00E5FF'} />}>
          <Row label={ui('communityLabel', 'Community Leaderboard')}
            desc={ui('communityDesc', 'See how you rank against other learners. Opt-in, chosen display name only.')}
            onPress={() => router.push('/community')} right={<ChevronRight size={16} color={C.textMuted} style={chevronStyle} />}
            accessibilityLabel={ui('communityLabel', 'Community Leaderboard')} />
        </Section>

        <Section title={t.helpSection} icon={<HelpCircle size={18} color={C.cyan || '#00E5FF'} />}>
          <Row label={t.helpSection} desc={t.helpDescription}
            onPress={() => router.push('/help')} right={<ChevronRight size={16} color={C.textMuted} style={chevronStyle} />}
            accessibilityLabel={t.helpSection} />
          <Row label={t.feedbackLabel} desc={t.feedbackDescription}
            onPress={() => router.push('/feedback')} right={<MessageSquare size={16} color={C.textMuted} />}
            accessibilityLabel={t.feedbackLabel} />
          <Row label={t.privacyLabel} desc={t.howWeHandleYourData}
            onPress={() => Linking.openURL('https://poly-puff.com/privacy')}
            right={<ExternalLink size={16} color={C.textMuted} />}
            accessibilityLabel={t.privacyLabel} />
          <Row label={t.termsOfService} desc={t.termsOfServiceDesc}
            onPress={() => Linking.openURL('https://poly-puff.com/terms')}
            right={<ExternalLink size={16} color={C.textMuted} />}
            accessibilityLabel={ui('viewTermsExternal', 'View terms of service. Opens external link.')} />
          <Row label={t.deleteAccountLabel} desc={t.permanentlyRemoveAccount}
            onPress={() => Linking.openURL('https://poly-puff.com/delete-account')}
            right={<ExternalLink size={16} color={C.red || '#FF4D6A'} />}
            danger
            accessibilityLabel={t.deleteAccountLabel} />
        </Section>

        {/* ── ACCOUNT & DATA ───────────────────────────────────────────── */}
        <Section title={t.accountSection} icon={<Trash2 size={18} color={C.red || '#FF4D6A'} />}>
          <Row
            label={t.clearLocalAppData}
            desc={t.clearLocalAppDataDesc}
            onPress={() => {
              Alert.alert(
                t.clearLocalAppData,
                t.clearLocalAppDataConfirmDesc,
                [
                  { text: t.cancel, style: 'cancel' },
                  { text: t.clearLocalDataAction, style: 'destructive', onPress: async () => {
                    try { await AsyncStorage.clear(); Alert.alert(t.localDataClearedTitle, t.localDataClearedMsg, [{ text: t.okLabel }]); }
                    catch (e) { Alert.alert(t.errorTitle, t.clearLocalDataErrorMsg); }
                  }},
                ]
              );
            }}
            right={<Trash2 size={16} color={C.red || '#FF4D6A'} />}
            danger
            accessibilityLabel={ui('clearLocalAppData', 'Clear local app data on this device')}
            accessibilityHint={t.clearLocalAppDataConfirmDesc}
          />
        </Section>

        {/* ── ABOUT ────────────────────────────────────────────────────── */}
        <Section title={t.aboutSection} icon={<Info size={18} color={C.textSec} />}>
          <Row label={t.appVersion} right={<Text style={{ fontSize: scaledFont(14), color: C.textMuted }}>{appVersion}</Text>}
            accessibilityLabel={tx('appVersionA11y', { version: appVersion })} />
          <Row label={t.backendVersion} right={<Text style={{ fontSize: scaledFont(14), color: C.textMuted }}>{backendVersion}</Text>}
            accessibilityLabel={tx('backendVersionA11y', { version: backendVersion })} />
          <Row label={t.grammarRulesLabel}
            right={<Text style={{ fontSize: scaledFont(14), color: C.textMuted }}>{(t.rulesAndCategories || '{rules} rules · {cats} categories').replace('{rules}', String(ruleStats.totalRules)).replace('{cats}', String(ruleStats.categoryCount))}</Text>}
            accessibilityLabel={tx('grammarDatabaseA11y', { rules: ruleStats.totalRules, cats: ruleStats.categoryCount })} />
          <Row label={t.languagesSupportedLabel}
            right={<Text style={{ fontSize: scaledFont(14), color: C.textMuted }}>{(t.languagesSupportedCount || '{n} supported').replace('{n}', String(LANGUAGE_OPTIONS.length))}</Text>}
            accessibilityLabel={tx('supportedLanguagesA11y', { count: LANGUAGE_OPTIONS.length })} />
          <Row label={t.cefrLevels} right={<Text style={{ fontSize: scaledFont(14), color: C.textMuted }}>A0 – C2</Text>}
            accessibilityLabel={t.cefrLevels} />
        </Section>

        <View style={{ height: 20 }} />
      </ScrollView>

      {/* ── TIME PICKER MODAL ─────────────────────────────────────────── */}
      <Modal visible={showTimePicker} transparent animationType="fade" onRequestClose={() => setShowTimePicker(false)}>
        <TouchableOpacity style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'flex-end' }} activeOpacity={1} onPress={() => setShowTimePicker(false)}>
          <View style={{ backgroundColor: C.card, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24, paddingBottom: 40, maxHeight: '60%' }}
            accessibilityViewIsModal={true}>
            <Text style={{ fontSize: scaledFont(18), fontWeight: '800', color: C.text, marginBottom: 16, textAlign: 'center' }}
              accessibilityRole="header">{ui('chooseReminderTime', 'Choose Reminder Time')}</Text>
            <ScrollView showsVerticalScrollIndicator={false}>
              {TIME_OPTIONS.map(({ hour, minute }) => {
                const isSelected = hour === reminderHour && minute === reminderMinute;
                return (
                  <TouchableOpacity
                    key={`${hour}-${minute}`}
                    style={{
                      paddingVertical: 14, paddingHorizontal: 16, borderRadius: 10, marginBottom: 4,
                      backgroundColor: isSelected ? (C.purple || '#B06CFF') + '20' : 'transparent',
                      borderWidth: isSelected ? 1 : 0, borderColor: (C.purple || '#B06CFF') + '40',
                      minHeight: 44,
                    }}
                    onPress={() => handleTimeSelect(hour, minute)}
                    accessibilityRole="radio"
                    accessibilityLabel={formatReminderTime(hour, minute)}
                    accessibilityState={{ selected: isSelected }}
                  >
                    <Text style={{
                      fontSize: scaledFont(16), fontWeight: isSelected ? '700' : '500',
                      color: isSelected ? (C.purple || '#B06CFF') : C.text, textAlign: 'center',
                    }}>
                      {formatReminderTime(hour, minute)}
                    </Text>
                  </TouchableOpacity>
                );
              })}
            </ScrollView>
            <TouchableOpacity
              style={{ paddingVertical: 12, alignItems: 'center', marginTop: 8, minHeight: 44 }}
              onPress={() => setShowTimePicker(false)}
              accessibilityRole="button" accessibilityLabel={t.cancel}
            >
              <Text style={{ fontSize: scaledFont(14), color: C.textMuted }}>{t.cancel}</Text>
            </TouchableOpacity>
          </View>
        </TouchableOpacity>
      </Modal>

      {/* ── ANDROID PLAY BILLING PAYWALL MODAL ────────────────────────── */}
      <Modal visible={paywallVisible} transparent animationType="fade" onRequestClose={() => setPaywallVisible(false)}>
        <TouchableOpacity style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'flex-end' }} activeOpacity={1} onPress={() => setPaywallVisible(false)}>
          <View style={{ backgroundColor: C.card, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24, paddingBottom: 40, maxHeight: '75%' }}
            accessibilityViewIsModal={true}>
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                <Sparkles size={20} color={C.purple || '#B06CFF'} />
                <Text style={{ fontSize: scaledFont(18), fontWeight: '800', color: C.text }} accessibilityRole="header">
                  {ui('upgradeYourPlan', 'Upgrade Your Plan')}
                </Text>
              </View>
              <TouchableOpacity
                onPress={() => setPaywallVisible(false)}
                style={{ padding: 6, minWidth: 32, minHeight: 32, alignItems: 'center', justifyContent: 'center' }}
                accessibilityRole="button"
                accessibilityLabel={t.cancel}
              >
                <X size={20} color={C.textMuted} />
              </TouchableOpacity>
            </View>

            {paywallLoading && paywallPackages.length === 0 ? (
              <View style={{ paddingVertical: 40, alignItems: 'center' }}>
                <ActivityIndicator size="large" color={C.purple || '#B06CFF'} />
              </View>
            ) : paywallPackages.length === 0 ? (
              <View style={{ paddingVertical: 24, alignItems: 'center' }}>
                <Text style={{ fontSize: scaledFont(14), color: C.textMuted, textAlign: 'center' }}>
                  {ui('noPlansAvailable', 'No plans are available right now. Please try again later.')}
                </Text>
              </View>
            ) : (
              <ScrollView showsVerticalScrollIndicator={false}>
                {(() => {
                  const teacherPkgs = paywallPackages.filter((pkg) => pkg.identifier.startsWith('teacher_'));
                  const proPkgs = paywallPackages.filter((pkg) => !pkg.identifier.startsWith('teacher_'));

                  // Play Console only lets us name the subscription itself, not each
                  // base plan - so "Pro Plan"/"Teacher Plan" is identical on both the
                  // monthly and annual row and only the price differs. Derive a period
                  // label from the package identifier (all four end in _monthly/_annual,
                  // whether reserved like $rc_monthly or custom like teacher_monthly) so
                  // testers/users can actually tell the two apart.
                  const periodSuffix = (id: string) => {
                    if (id.includes('annual')) return ui('perYear', '/yr');
                    if (id.includes('monthly')) return ui('perMonth', '/mo');
                    return '';
                  };

                  const renderPackage = (pkg: PurchasesPackage) => {
                    const isPurchasing = purchasingId === pkg.identifier;
                    const disabled = !!purchasingId;
                    return (
                      <TouchableOpacity
                        key={pkg.identifier}
                        disabled={disabled}
                        onPress={() => handlePurchase(pkg)}
                        style={{
                          flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between',
                          paddingVertical: 14, paddingHorizontal: 16, borderRadius: 12, marginBottom: 8,
                          backgroundColor: (C.purple || '#B06CFF') + '14',
                          borderWidth: 1, borderColor: (C.purple || '#B06CFF') + '30',
                          opacity: disabled && !isPurchasing ? 0.5 : 1,
                          minHeight: 56,
                        }}
                        accessibilityRole="button"
                        accessibilityLabel={`${pkg.product.title} - ${pkg.product.priceString}${periodSuffix(pkg.identifier)}`}
                        accessibilityState={{ disabled, busy: isPurchasing }}
                      >
                        <View style={{ flex: 1, marginRight: 12 }}>
                          <Text style={{ fontSize: scaledFont(15), fontWeight: '700', color: C.text }}>{pkg.product.title}</Text>
                          {!!pkg.product.description && (
                            <Text style={{ fontSize: scaledFont(12), color: C.textMuted, marginTop: 2 }} numberOfLines={2}>
                              {pkg.product.description}
                            </Text>
                          )}
                        </View>
                        {isPurchasing ? (
                          <ActivityIndicator size="small" color={C.purple || '#B06CFF'} />
                        ) : (
                          <Text style={{ fontSize: scaledFont(16), fontWeight: '800', color: C.purple || '#B06CFF' }}>
                            {pkg.product.priceString}
                            <Text style={{ fontSize: scaledFont(12), fontWeight: '600', color: C.textMuted }}>
                              {periodSuffix(pkg.identifier)}
                            </Text>
                          </Text>
                        )}
                      </TouchableOpacity>
                    );
                  };

                  return (
                    <>
                      {proPkgs.length > 0 && (
                        <>
                          <Text style={{ fontSize: scaledFont(12), fontWeight: '700', color: C.textMuted, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 8 }}>
                            {t.proPlan}
                          </Text>
                          {proPkgs.map(renderPackage)}
                        </>
                      )}
                      {teacherPkgs.length > 0 && (
                        <>
                          <Text style={{ fontSize: scaledFont(12), fontWeight: '700', color: C.textMuted, textTransform: 'uppercase', letterSpacing: 0.5, marginTop: proPkgs.length > 0 ? 16 : 0, marginBottom: 8 }}>
                            {ui('teacherPlanBadge', 'Teacher')}
                          </Text>
                          <Text style={{ fontSize: scaledFont(12), color: C.textMuted, marginBottom: 10 }}>
                            {ui('teacherPlanDesc', 'Everything in Pro, plus classroom tools: create student groups, track class progress, and assign lessons from the website.')}
                          </Text>
                          {teacherPkgs.map(renderPackage)}
                        </>
                      )}
                    </>
                  );
                })()}
              </ScrollView>
            )}

            <TouchableOpacity
              style={{ paddingVertical: 12, alignItems: 'center', marginTop: 8, minHeight: 44 }}
              onPress={handleRestorePurchases}
              disabled={paywallLoading || !!purchasingId}
              accessibilityRole="button"
              accessibilityLabel={ui('restorePurchases', 'Restore purchases')}
            >
              <Text style={{ fontSize: scaledFont(14), color: C.textMuted }}>
                {ui('restorePurchases', 'Restore Purchases')}
              </Text>
            </TouchableOpacity>
          </View>
        </TouchableOpacity>
      </Modal>

      {renderPreferencePicker()}

    </ScreenBackground>
  );
}
