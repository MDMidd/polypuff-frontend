/**
 * Settings Screen - Poly-Puff v6.3
 * =================================================
 *
 * CHANGES IN v6.3:
 *   - Added DigestSettings component (Weekly Digest section)
...
import {
  ...
  Switch, Alert, Linking, Modal, TextInput, type GestureResponderEvent,
} from 'react-native';
...
const NATIVE_LANGUAGES = [
  'Afrikaans', 'Amharic', 'Arabic', 'Bengali', 'Bulgarian', 'Czech', 'Danish',
  'Dutch', 'Filipino', 'Finnish', 'French', 'German', 'Greek', 'Guarani', 'Gujarati',
  'Haitian Creole', 'Hausa', 'Hebrew', 'Hindi', 'Hungarian', 'Igbo', 'Indonesian',
  'Italian', 'Japanese', 'Korean', 'Malay', 'Mandarin', 'Marathi', 'Nepali',
  'Norwegian', 'Pashto', 'Persian', 'Polish', 'Portuguese', 'Punjabi', 'Quechua',
  'Romanian', 'Russian', 'Sinhala', 'Slovak', 'Spanish', 'Swahili', 'Swedish',
  'Tamil', 'Telugu', 'Thai', 'Turkish', 'Ukrainian', 'Urdu', 'Vietnamese',
  'Yoruba', 'Zulu',
];

const APP_LANGUAGES: Array<{ code: LangCode; label: string }> = [
  ...
  { code: 'gn', label: 'Guarani' }, { code: 'gu', label: 'Gujarati' },
  { code: 'ht', label: 'Haitian Creole' }, { code: 'ha', label: 'Hausa' },
  ...
  { code: 'zh', label: 'Mandarin' }, { code: 'mr', label: 'Marathi' },
  ...
  { code: 'pa', label: 'Punjabi' }, { code: 'qu', label: 'Quechua' },
...
const CEFR_LEVELS = [
  { code: 'A0', label: 'Pre-A1 starter', desc: 'No prior knowledge, basic words' },
  { code: 'A1', label: 'Beginner', desc: 'Basic words and phrases' },
  { code: 'A2', label: 'Elementary', desc: 'Simple sentences and familiar topics' },
  { code: 'B1', label: 'Intermediate', desc: 'Opinions, everyday problems, common tenses' },
  { code: 'B2', label: 'Upper-intermediate', desc: 'Complex sentences, nuance, idioms' },
  { code: 'C1', label: 'Advanced', desc: 'Flexible, sophisticated language' },
  { code: 'C2', label: 'Proficient', desc: 'Near-native precision' },
];

type PickerMode = 'nativeLanguage' | 'appLanguage' | 'cefrLevel' | null;
...
export default function SettingsScreen() {
  ...
  const { t, wt, setLang } = useLanguage();
  const ui = (key: keyof typeof t, fallback: string) => (t[key] as string | undefined) ?? fallback;
  const brandName = 'Poly-Puff';
  const serverUrlPlaceholder = 'http://192.168.1.10:3000';
  const mascotCtx: MascotContextValue = (() => { try { return useMascot(); } catch { return { mascotEnabled: true, setMascotEnabled: (_value: boolean) => {} }; } })();
  ...
  const [activeModuleCount, setActiveModuleCount] = useState(9);
  ...
  const [accountPlan, setAccountPlan] = useState('Not synced');
  ...
  const [billingDesc, setBillingDesc] = useState('Sign in on the web to sync billing details.');
  const [promptsUsed, setPromptsUsed] = useState('-');
  const [promptDesc, setPromptDesc] = useState('Sign in on the web to track prompt usage.');
  ...
  const [emailReminderDesc, setEmailReminderDesc] = useState('Sign in on the web to manage email learning reminders.');
  ...
  const normalizedAccountMethod = String(accountMethod || '').toLowerCase();
  const canChangePassword = !normalizedAccountMethod || PASSWORD_RESET_PROVIDERS.has(normalizedAccountMethod);
  ...
  useEffect(() => {
    ...
    // Stats: use hardcoded values as primary source.
    // Try to get live counts from backend; only update if the values are non-zero.
    getServerUrl().then(BASE =>
      fetch(`${BASE}/api/stats`)
        .then(r => r.json())
        .then(data => {
          const liveRules    = data.totalRules || 0;
          const liveCats     = data.categories ? Object.keys(data.categories).length : 0;
          const liveLangs    = data.languages  ? data.languages.length : 0;
          setRuleStats({
            totalRules:    liveRules > 0 ? liveRules : 340,
            categoryCount: liveCats  > 0 ? liveCats  : 35,
            languageCount: liveLangs > 0 ? liveLangs : 14,
          });
        })
        .catch(() => {}) // keep defaults on any network failure
    );
    getServerUrl()
      .then(() => checkHealth())
      .then(health => setBackendVersion(health?.version ? String(health.version) : '-'))
  ...
  const loadAccountSummary = async () => {
    try {
      ...
      const [
        profileRaw, authIdentifier, authMethod, digestEmail,
        savedToken, cachedAccount,
      ] = await Promise.all([
        ...
        getSavedWebToken(),
        AsyncStorage.getItem('accountSummary'),
      ]);

      let profileEmail = '';
      if (profileRaw) {
        const profile = JSON.parse(profileRaw);
        profileEmail = profile.email || profile.userEmail || profile.authIdentifier || '';
      }

      const email = [profileEmail, authIdentifier, digestEmail].find(v => v && v.includes('@')) || '';
      setAccountEmail(email);
      setAccountMethod(authMethod || (email ? 'email' : ''));

      if (cachedAccount) {
        applyAccountSummary(JSON.parse(cachedAccount));
      }

      if (savedToken) {
        setAccountToken(savedToken);
        loadEmailReminderPreference(savedToken);
        await refreshAccountSummary(savedToken);
      } else if (!cachedAccount) {
        setEmailReminderEnabled(false);
        setEmailReminderDesc('Sign in on the web to manage email learning reminders.');
        setLocalAccountSummary(email);
      }
    } catch {
      setAccountEmail('');
      setAccountMethod('');
      setEmailReminderEnabled(false);
      setEmailReminderDesc('Sign in on the web to manage email learning reminders.');
      setLocalAccountSummary('');
    } finally {
      setLoadingAccount(false);
    }
  };

  const normaliseCefrLevel = (level?: string) => {
    const clean = String(level || '').trim().toUpperCase();
    return CEFR_LEVELS.some(item => item.code === clean) ? clean : 'B1';
  };

  const getAppLanguageLabel = (code: LangCode) => APP_LANGUAGES.find(lang => lang.code === code)?.label || 'English';
  const getCefrLabel = (code: string) => {
    const level = CEFR_LEVELS.find(item => item.code === normaliseCefrLevel(code)) || CEFR_LEVELS[3];
  ...
  const loadLearningPrefs = async () => {
    try {
      ...
      const savedAppLanguage = APP_LANGUAGES.some(lang => lang.code === profile.appLanguage) ? profile.appLanguage : 'en';
      setNativeLanguage(profile.nativeLanguage || 'English');
      setAppLanguage(savedAppLanguage);
      setCefrLevel(normaliseCefrLevel(profile.level || profile.cefrLevel));
    } catch {
  ...
  const saveProfilePatch = async (patch: Record<string, unknown>) => {
    const raw = await AsyncStorage.getItem('userProfile');
    const profile = raw ? JSON.parse(raw) : {};
    await AsyncStorage.setItem('userProfile', JSON.stringify({ ...profile, ...patch }));
  };

  const saveNativeLanguage = async (value: string) => {
    setNativeLanguage(value);
    await saveProfilePatch({ nativeLanguage: value });
    setPickerMode(null);
  };

  const saveAppLanguage = async (value: LangCode) => {
    setAppLanguage(value);
    await setLang(value);
    await saveProfilePatch({ appLanguage: value });
    setPickerMode(null);
  };

  const saveCefrLevel = async (value: string) => {
    const level = normaliseCefrLevel(value);
    setCefrLevel(level);
    await saveProfilePatch({ level, cefrLevel: level });
    setPickerMode(null);
  };

  // Reload active module count whenever Settings gets focus
  useFocusEffect(
    React.useCallback(() => {
      AsyncStorage.getItem('practiceModuleConfig').then(raw => {
        if (raw) {
          try {
            const cfg = JSON.parse(raw);
            setActiveModuleCount(cfg.active ? cfg.active.length : 9);
          } catch {}
        }
      }).catch(() => {});
    }, [])
  ...
  const handleSoundToggle  = async (val: boolean) => { setSoundOn(val);  await setSoundEnabled(val);  };
  const handleHapticToggle = async (val: boolean) => { setHapticOn(val); await setHapticEnabled(val); };

  const handleReminderToggle = async (val: boolean) => {
    setReminderEnabled(val);
    await setReminderSettings(val, reminderHour, reminderMinute);
  };
  ...
  const handleTimeSelect = async (hour: number, minute: number) => {
    ...
    if (reminderEnabled) await setReminderSettings(true, hour, minute);
  };
  ...
  const testServerConnection = async () => {
    ...
    try {
      ...
      const label = health?.rulesLoaded ? `Connected - ${health.rulesLoaded} rules` : 'Connected';
      setServerStatus(label);
      Alert.alert('Connection OK', 'Server connection is healthy.');
      return true;
    } catch (e) {
      setServerStatus('Not reachable');
      Alert.alert('Connection Failed', 'Poly-Puff could not reach this backend URL.');
      return false;
    }
  };

  const chooseServerUrl = async (url: string, label: string) => {
    await setServerUrl(url);
    setCurrentServerUrl(url);
    setServerUrlInput(url);
    setServerStatus('Not tested');
    Alert.alert('Backend Updated', `Using ${label}.`);
  };

  const saveCustomServerUrl = async () => {
    ...
    setCurrentServerUrl(trimmed);
    setServerUrlInput(trimmed);
    setServerStatus('Not tested');
    await testServerConnection();
  };

  const getSavedWebToken = async () => {
    const tokenKeys = ['pp_token', 'webAuthToken', 'authToken', 'jwt', 'polyPuffToken'];
    const entries = await AsyncStorage.multiGet(tokenKeys);
  ...
  const setLocalAccountSummary = (email: string) => {
    setAccountPlan(email ? 'Not synced' : 'Local');
    setAccountPlanTone('local');
    setBillingStatus('Not synced');
    setBillingDesc(email ? 'Web account details are not synced on this device yet.' : 'No web account is saved on this device.');
    setPromptsUsed('-');
    setPromptDesc(email ? 'Prompt usage syncs after web sign-in.' : 'Sign in on the web to track prompt usage.');
  ...
  const applyAccountSummary = (account: any) => {
    ...
    if (account?.provider) setAccountMethod(String(account.provider));

    if (account?.isAdmin) {
      setAccountPlan('Admin');
    ...
    setPromptDesc(limit ? `${Math.max(0, limit - used)} prompt${Math.max(0, limit - used) === 1 ? '' : 's'} remaining today` : 'Unlimited prompts');
    ...
    if (billing.paddleStatus === 'active') {
      ...
      setBillingDesc(periodEnd ? `Renews on ${periodEnd}` : 'Subscription is active.');
    } else if (billing.paddleStatus === 'trialing') {
      setBillingStatus('Trial');
      setBillingDesc(periodEnd ? `Trial ends on ${periodEnd}` : 'Trial subscription is active.');
    } else if (billing.paddleStatus === 'past_due') {
      setBillingStatus('Past due');
      setBillingDesc('Payment needs attention.');
    } else if (billing.paddleStatus === 'canceled' || billing.paddleStatus === 'cancelled') {
      setBillingStatus('Canceled');
      setBillingDesc(cancelAt ? `Canceled on ${cancelAt}` : 'Subscription is canceled.');
    } else if (account?.isPro || account?.isAdmin) {
      setBillingStatus(status || 'Active');
      setBillingDesc(periodEnd ? `Current period ends on ${periodEnd}` : 'Billing details synced.');
    } else {
      setBillingStatus('Free');
      setBillingDesc('No active subscription.');
    }
  };
  ...
  const refreshAccountSummary = async (token = accountToken) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || 'Could not load account info.');
      await AsyncStorage.setItem('accountSummary', JSON.stringify(data));
      applyAccountSummary(data);
    } catch {
      setBillingDesc('Could not refresh web account details.');
    } finally {
      setLoadingAccount(false);
    }
  };

  const loadEmailReminderPreference = async (token = accountToken) => {
    if (!token) {
      setEmailReminderEnabled(false);
      setEmailReminderDesc('Sign in on the web to manage email learning reminders.');
      return;
    ...
    try {
      const BASE = await getServerUrl();
      const res = await fetch(`${BASE}/api/reminders/preferences`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data.error || 'Could not load email reminder settings.');

      setEmailReminderEnabled(!!data.enabled);
      setEmailReminderDesc(data.enabled
        ? 'Poly-Puff can send occasional AI-personalized reminders based on your recent progress.'
        : 'Turn this on to receive occasional AI-personalized reminders based on your recent progress.');
    } catch {
      setEmailReminderDesc('Could not load email reminder settings.');
    } finally {
      setEmailReminderLoading(false);
    }
  };

  const saveEmailReminderPreference = async (enabled: boolean) => {
    if (!accountToken) {
      setEmailReminderEnabled(false);
      Alert.alert('Web Sign-In Needed', 'Sign in on the web to manage email learning reminders.');
      return;
    }

    const previous = emailReminderEnabled;
    setEmailReminderEnabled(enabled);
    setEmailReminderLoading(true);
    try {
      const BASE = await getServerUrl();
      const res = await fetch(`${BASE}/api/reminders/preferences`, {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data.error || 'Could not save email reminder settings.');
      ...
      setEmailReminderEnabled(previous);
      Alert.alert('Email Reminders', 'Could not save email reminder setting. Please try again.');
    } finally {
      setEmailReminderLoading(false);
    }
  };
  ...
  const openBillingPortal = async () => {
    if (!accountToken) {
      Alert.alert('Web Sign-In Needed', 'Billing management is available after your web account token is synced on this device.');
      return;
    }
    ...
    try {
      const BASE = await getServerUrl();
      const res = await fetch(`${BASE}/api/paddle/portal-session`, {
        method: 'POST',
      ...
      } else if (res.status === 404 && data.supportEmail) {
        await Linking.openURL('mailto:support@poly-puff.com?subject=Billing%20Help&body=Please%20help%20me%20manage%20my%20Poly-Puff%20billing.');
      } else {
        Alert.alert('Billing Portal', data.error || 'Could not open the billing portal.');
      }
    } catch {
      Alert.alert('Billing Portal', 'Could not open the billing portal. Please try again in a moment.');
    } finally {
  ...
  const openPricing = () => {
    Linking.openURL('https://polypuff.app/pricing');
  };

  const signOut = () => {
    Alert.alert('Sign Out', 'This removes saved account tokens from this device. Your profile and learning progress stay here.', [
      { text: t.cancel, style: 'cancel' },
      { text: 'Sign Out', style: 'destructive', onPress: async () => {
        await AsyncStorage.multiRemove([
          'loginComplete', 'loginDate', 'authMethod', 'authIdentifier', 'authPhotoURL',
          'pp_token', 'webAuthToken', 'authToken', 'jwt', 'polyPuffToken', 'accountSummary',
        ]);
        setAccountToken('');
        setAccountEmail('');
        setAccountMethod('');
        ...
        setEmailReminderEnabled(false);
        setEmailReminderDesc('Sign in on the web to manage email learning reminders.');
        Alert.alert('Signed Out', 'Saved account details were removed from this device.');
        router.replace('/');
      }},
    ]);
  };
  ...
  const sendPasswordReset = async () => {
    if (!accountEmail) {
      Alert.alert('No Email Found', 'Sign in with an email address before requesting a password reset.');
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        Alert.alert('Reset Link Sent', `Check ${accountEmail} for your password reset link.`);
      } else {
        Alert.alert('Could Not Send Reset', data.error || 'Please try again in a moment.');
      }
    } catch {
  ...
  const TIME_OPTIONS: Array<{ hour: number; minute: number }> = [];
  for (let h = 6; h <= 22; h++) {
  ...
  const estimateCache = async () => {
    try {
      ...
      let total = 0;
      for (const key of keys) { const val = await AsyncStorage.getItem(key); if (val) total += val.length; }
    ...
    } catch (e) { setCacheSize('Unknown'); }
  };

  const clearCache = () => {
    Alert.alert('Clear Cache', 'This removes cached rules and exercises. They will re-download next time you open the app.', [
      { text: t.cancel, style: 'cancel' },
      { text: 'Clear', style: 'destructive', onPress: async () => { await AsyncStorage.multiRemove(['rulesCache_all_all', 'exerciseCache']); setCacheSize('0 bytes'); }},
    ]);
  };
  ...
  const clearAllProgress = () => {
    Alert.alert('Clear All Progress', 'This will permanently delete ALL your exercise history, XP, badges, and streak data. This cannot be undone.', [
      ...
      { text: 'Delete Everything', style: 'destructive', onPress: async () => {
        await AsyncStorage.multiRemove(['mistakeHistory', 'totalXP', 'sessionHistory', 'completedChallenges']);
  ...
  const resetDefaults = () => {
    Alert.alert(
      'Set to Default?',
      'Resets language, CEFR level, and UI preferences to their defaults. Your account, progress, vaults, and bookmarks are kept.',
      [
        { text: t.cancel, style: 'cancel' },
        { text: 'Reset', style: 'destructive', onPress: async () => {
          await AsyncStorage.multiRemove([
            'soundEnabled', 'hapticEnabled', 'mascotEnabled',
            'streakReminderEnabled', 'streakReminderHour', 'streakReminderMinute',
            'practiceModuleConfig',
          ]);

          const saved = await AsyncStorage.getItem('userProfile');
          if (saved) {
            const profile = JSON.parse(saved);
            delete profile.nativeLanguage;
            delete profile.level;
            delete profile.appLanguage;
            delete profile.cefrLevel;
          ...
          setReminderHour(18);
          setReminderMinute(0);
          setActiveModuleCount(9);
          setNativeLanguage('English');
          setAppLanguage('en');
          setCefrLevel('B1');
          await setLang('en');
          mascotCtx.setMascotEnabled(true);

          Alert.alert('Defaults Restored', 'Language, CEFR level, and UI preferences were reset. Your account, progress, vaults, and bookmarks were kept.');
        }},
      ]
    );
  };
  ...
  const clearProfile = () => {
    Alert.alert('Clear Profile', 'This will remove your name, profession, hobbies, and profile picture. Language and level will be kept.', [
      ...
      { text: 'Clear Profile', style: 'destructive', onPress: async () => {
        const saved = await AsyncStorage.getItem('userProfile');
        if (saved) { const p = JSON.parse(saved); await AsyncStorage.setItem('userProfile', JSON.stringify({ nativeLanguage: p.nativeLanguage, level: p.level })); }
  ...
  const Section = ({ title, icon, children }: SectionProps) => (
    <View style={{ backgroundColor: C.card, borderRadius: 16, marginBottom: 12, borderWidth: 1, borderColor: C.border + '15', overflow: 'hidden' }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, padding: 16, paddingBottom: 8 }}>
        {icon}
        <Text style={{ fontSize: scaledFont(15), fontWeight: '700', color: C.text }} accessibilityRole="header">{title}</Text>
      </View>
      {children}
    </View>
  );

  const Row = ({ label, desc, right, onPress, danger, accessibilityLabel: a11yLabel, accessibilityHint: a11yHint }: RowProps) => (
    <TouchableOpacity
      style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingVertical: 14, borderTopWidth: 1, borderTopColor: C.border + '10', minHeight: 44 }}
      onPress={onPress}
      disabled={!onPress}
    ...
    >
      <View style={{ flex: 1, marginRight: 12 }}>
        <Text style={{ fontSize: scaledFont(15), color: danger ? C.red : C.text }}>{label}</Text>
        {desc && <Text style={{ fontSize: scaledFont(12), color: C.textMuted, marginTop: 2 }}>{desc}</Text>}
      </View>
      {right}
    </TouchableOpacity>
  );

  const PlanBadge = ({ label, tone }: { label: string; tone: 'free' | 'pro' | 'admin' | 'local' }) => {
    const palette = {
      free: { bg: (C.cardAlt || C.card) + '80', fg: C.textMuted, border: C.border + '40' },
      pro: { bg: (C.emerald || '#00E5A0') + '20', fg: C.emerald || '#00E5A0', border: (C.emerald || '#00E5A0') + '40' },
      admin: { bg: (C.purple || '#B06CFF') + '20', fg: C.purple || '#B06CFF', border: (C.purple || '#B06CFF') + '40' },
      local: { bg: (C.cyan || '#00E5FF') + '12', fg: C.textMuted, border: C.border + '30' },
    }[tone];
    ...
    return (
      <View style={{ backgroundColor: palette.bg, borderColor: palette.border, borderWidth: 1, paddingHorizontal: 10, paddingVertical: 4, borderRadius: 999 }}>
        <Text style={{ fontSize: scaledFont(11), fontWeight: '800', color: palette.fg }}>{label}</Text>
      </View>
    );
  };
  ...
  const renderPreferencePicker = () => {
    ...
    const title = pickerMode === 'nativeLanguage'
      ? t.nativeLanguage
      : pickerMode === 'appLanguage'
        ? t.appLanguage
        : `CEFR ${t.level}`;
    const options = pickerMode === 'nativeLanguage'
      ? NATIVE_LANGUAGES.map(name => ({ key: name, label: name, desc: 'Grammar tips and translations use this language' }))
      : pickerMode === 'appLanguage'
        ? APP_LANGUAGES.map(lang => ({ key: lang.code, label: lang.label, desc: lang.code.toUpperCase() }))
        : CEFR_LEVELS.map(level => ({ key: level.code, label: `${level.code} - ${level.label}`, desc: level.desc }));
    const selected = pickerMode === 'nativeLanguage' ? nativeLanguage : pickerMode === 'appLanguage' ? appLanguage : cefrLevel;

    const handleSelect = async (key: string) => {
      if (pickerMode === 'nativeLanguage') await saveNativeLanguage(key);
      else if (pickerMode === 'appLanguage') await saveAppLanguage(key as LangCode);
      else await saveCefrLevel(key);
    };

    return (
      <Modal visible transparent animationType="fade" onRequestClose={() => setPickerMode(null)}>
        <TouchableOpacity style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'flex-end' }} activeOpacity={1} onPress={() => setPickerMode(null)}>
          <View style={{ backgroundColor: C.card, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 20, paddingBottom: 36, maxHeight: '72%' }}
            accessibilityViewIsModal={true}>
            <Text style={{ fontSize: scaledFont(18), fontWeight: '800', color: C.text, marginBottom: 14, textAlign: 'center' }}
                ...
                return (
                  <TouchableOpacity
                    ...
                    style={{
                      ...
                      paddingVertical: 13, paddingHorizontal: 14, borderRadius: 10, marginBottom: 4,
                      backgroundColor: isSelected ? (C.cyan || '#00E5FF') + '18' : 'transparent',
                      borderWidth: isSelected ? 1 : 0, borderColor: (C.cyan || '#00E5FF') + '40',
                      minHeight: 44,
                    }}
                  ...
                  >
                    <View style={{ flex: 1 }}>
                      <Text style={{ fontSize: scaledFont(15), fontWeight: isSelected ? '800' : '600', color: isSelected ? (C.cyan || '#00E5FF') : C.text }}>
                        {option.label}
                      </Text>
                      <Text style={{ fontSize: scaledFont(12), color: C.textMuted, marginTop: 2 }}>{option.desc}</Text>
                    </View>
                    {isSelected && <CheckCircle size={17} color={C.cyan || '#00E5FF'} />}
                  </TouchableOpacity>
                );
              })}
            </ScrollView>
  ...
  return (
    <ScreenBackground style={{ flex: 1 }}>
      <BackHeader title={t.settings} subtitle={undefined} onPress={undefined} rightElement={undefined} style={undefined} />
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 40 }}>
        <Image
          ...
          style={{ height: 36, width: 180, resizeMode: 'contain', marginTop: 10 }}
          accessibilityLabel={brandName}
        />
        <Text style={{ fontSize: scaledFont(13), color: C.textSec, marginTop: 2, marginBottom: 20 }}>{wt('webapp-settings-desc')}</Text>

        <Section title={wt('account')} icon={<User size={18} color={C.cyan || '#00E5FF'} />}>
          <Row
            label="Email"
            desc={accountEmail || 'No email saved on this device'}
            right={<Text style={{ fontSize: scaledFont(13), color: C.textMuted }}>{accountMethod || 'Local'}</Text>}
            accessibilityLabel={`Account email: ${accountEmail || 'not saved'}`}
          />
          <Row
            label="Plan"
            desc={loadingAccount ? 'Refreshing account details...' : undefined}
            right={
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                ...
                {canUpgradePlan && (
                  <TouchableOpacity
                    ...
                    accessibilityRole="button"
                    accessibilityLabel={wt('view-pricing')}
                  >
                    <Text style={{ fontSize: scaledFont(11), fontWeight: '800', color: '#06111F' }}>{wt('view-pricing')}</Text>
                  </TouchableOpacity>
                )}
              </View>
            }
            accessibilityLabel={`Account plan: ${accountPlan}`}
          />
          <Row
            label="Billing Status"
            desc={billingDesc}
            right={<Text style={{ fontSize: scaledFont(13), color: C.textMuted, textTransform: 'capitalize' }}>{billingStatus}</Text>}
            accessibilityLabel={`Billing status: ${billingStatus}`}
          />
          <Row
            label="Prompts Used Today"
            desc={promptDesc}
            right={<Text style={{ fontSize: scaledFont(13), color: C.textMuted }}>{promptsUsed}</Text>}
            accessibilityLabel={`Prompts used today: ${promptsUsed}`}
          />
          {promptUsage.synced && (
            <View
              style={{
                ...
                paddingBottom: 14,
                borderTopWidth: 1,
                borderTopColor: C.border + '10',
              }}
              accessibilityLabel={promptUsage.limit
                ? `Prompt usage ${Math.round(promptBarPercent)} percent`
                : 'Prompt usage unlimited'}
            >
              <View style={{ height: 6, borderRadius: 999, overflow: 'hidden', backgroundColor: C.border + '25' }}>
                <View
                  style={{
                    height: '100%',
                    width: `${promptBarPercent}%`,
                    backgroundColor: promptLimitReached ? C.red || '#FF4D6A' : C.cyan || '#00E5FF',
                  }}
                />
              </View>
            </View>
          ...
          <Row
            label={ui('emailLearningReminders', 'Email Learning Reminders')}
            desc={emailReminderLoading ? 'Loading email reminder preference...' : emailReminderDesc}
            right={
              <Switch
                value={emailReminderEnabled}
                onValueChange={saveEmailReminderPreference}
                disabled={!accountToken || emailReminderLoading}
                trackColor={{ false: '#D1D5DB', true: (C.cyan || '#00E5FF') + '50' }}
                thumbColor={emailReminderEnabled ? C.cyan || '#00E5FF' : C.textMuted}
                accessibilityRole="switch"
                accessibilityLabel={ui('emailLearningReminders', 'Email learning reminders')}
                accessibilityState={{ checked: emailReminderEnabled, disabled: !accountToken || emailReminderLoading }}
              />
            }
            accessibilityLabel={`${ui('emailLearningReminders', 'Email learning reminders')} ${emailReminderEnabled ? 'on' : 'off'}`}
          />
          <Row
            label="Refresh Account"
            desc="Reload plan, billing, prompt usage, and email reminders"
            onPress={async () => {
              await refreshAccountSummary();
              await loadEmailReminderPreference();
            }}
            right={<RefreshCw size={16} color={C.textMuted} />}
            accessibilityLabel={ui('refreshAccountDetails', 'Refresh account details')}
          />
          {canChangePassword && (
            <Row
              ...
              right={resettingPassword
                ? <Text style={{ fontSize: scaledFont(13), color: C.textMuted }}>{ui('sending', 'Sending...')}</Text>
                : <KeyRound size={16} color={accountEmail ? C.textMuted : C.border || C.textMuted} />}
              accessibilityLabel={ui('sendPasswordReset', 'Send a password reset email')}
              accessibilityHint="Uses your saved account email address"
            />
          )}
          <Row
            label="Sign Out"
            desc="Remove saved account tokens from this device"
            onPress={signOut}
            right={<LogOut size={16} color={C.red || '#FF4D6A'} />}
            danger
            accessibilityLabel={ui('signOutDevice', 'Sign out of this device')}
          />
        </Section>

        <Section title="Billing" icon={<CreditCard size={18} color={C.emerald || '#00E5A0'} />}>
          <Row
            label="Manage Billing"
            desc="Open Paddle to manage your subscription, payment method, invoices, or cancellation"
          ...
          <Row
            ...
            desc="Read the refund policy or request a refund through support@poly-puff.com."
            onPress={() => Linking.openURL('https://polypuff.app/refund')}
        ...
        <Section title={`${wt('languages')} & ${t.level}`} icon={<Globe size={18} color={C.cyan || '#00E5FF'} />}>
          <Row
            label={t.nativeLanguage}
            desc="Grammar tips and translations use this language"
            onPress={() => setPickerMode('nativeLanguage')}
            right={<Text style={{ fontSize: scaledFont(13), color: C.textMuted }}>{nativeLanguage}</Text>}
            accessibilityLabel={`${t.nativeLanguage}: ${nativeLanguage}`}
            accessibilityHint="Opens native language picker"
          />
          <Row
            label={t.appLanguage}
            desc="Change the interface language"
            onPress={() => setPickerMode('appLanguage')}
            right={
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                <Languages size={14} color={C.textMuted} />
                <Text style={{ fontSize: scaledFont(13), color: C.textMuted }}>{getAppLanguageLabel(appLanguage)}</Text>
              </View>
            }
            accessibilityLabel={`${t.appLanguage}: ${getAppLanguageLabel(appLanguage)}`}
            accessibilityHint="Opens app language picker"
          />
          <Row
            label={`CEFR ${t.level}`}
            desc="Affects exercise difficulty"
            onPress={() => setPickerMode('cefrLevel')}
            right={
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                <BookOpen size={14} color={C.textMuted} />
                <Text style={{ fontSize: scaledFont(13), color: C.textMuted }}>{getCefrLabel(cefrLevel)}</Text>
              </View>
            }
            accessibilityLabel={`CEFR ${t.level}: ${getCefrLabel(cefrLevel)}`}
            accessibilityHint="Opens CEFR level picker"
          />
        </Section>
        ...
        <Section title={t.practice} icon={<Text style={{ fontSize: 18 }}>✏️</Text>}>
          <Row
            label={t.customisePracticeList || wt('customise-practice')}
            desc="Choose exercises, set order, and apply goal bundles"
            onPress={() => router.push('/customise')}
            right={
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                <View style={{ backgroundColor: (C.emerald || '#00E5A0') + '20', paddingHorizontal: 8, paddingVertical: 3, borderRadius: 8 }}>
                  <Text style={{ fontSize: scaledFont(11), fontWeight: '700', color: C.emerald || '#00E5A0' }}>
            ...
            accessibilityHint="Opens the Practice List customiser"
          />
        </Section>

        <Section title={ui('backend', 'Backend')} icon={<Server size={18} color={C.cyan || '#00E5FF'} />}>
          <Row
            label={ui('currentServer', 'Current Server')}
            desc={currentServerUrl || 'Loading...'}
            right={<Text style={{ fontSize: scaledFont(12), color: C.textMuted }}>{serverStatus}</Text>}
            accessibilityLabel={`Current backend server: ${currentServerUrl || 'loading'}`}
          />
          <View style={{ paddingHorizontal: 16, paddingVertical: 12, borderTopWidth: 1, borderTopColor: C.border + '10' }}>
            <TextInput
              style={{
              ...
              autoCorrect={false}
              keyboardType="url"
              accessibilityLabel={ui('customBackendServerUrl', 'Custom backend server URL')}
            />
            <TouchableOpacity
              style={{ backgroundColor: C.cyan || '#00E5FF', borderRadius: 10, paddingVertical: 11, alignItems: 'center', minHeight: 44 }}
              onPress={saveCustomServerUrl}
              accessibilityRole="button"
              accessibilityLabel={ui('saveTestBackendUrl', 'Save and test custom backend URL')}
            >
              <Text style={{ fontSize: scaledFont(14), fontWeight: '700', color: '#07111F' }}>{ui('saveAndTest', 'Save & Test')}</Text>
            </TouchableOpacity>
          </View>
          <Row
            label="Use Local Desktop"
            desc={LOCAL_SERVER}
            onPress={() => chooseServerUrl(LOCAL_SERVER, 'local desktop backend')}
            right={<ChevronRight size={16} color={C.textMuted} />}
            accessibilityLabel={ui('useLocalBackend', 'Use local desktop backend')}
          />
          <Row
            label="Use Android Emulator"
            desc={ANDROID_EMULATOR_SERVER}
            onPress={() => chooseServerUrl(ANDROID_EMULATOR_SERVER, 'Android emulator backend')}
            right={<ChevronRight size={16} color={C.textMuted} />}
            accessibilityLabel={ui('useEmulatorBackend', 'Use Android emulator backend')}
          />
          <Row
            label="Use Production"
            desc={DEFAULT_SERVER}
            onPress={() => chooseServerUrl(DEFAULT_SERVER, 'production backend')}
            right={<ChevronRight size={16} color={C.textMuted} />}
            accessibilityLabel={ui('useProductionBackend', 'Use production backend')}
          />
          <Row
            label="Test Connection"
            desc="Checks /api/health on the selected backend"
            onPress={testServerConnection}
            right={<ChevronRight size={16} color={C.textMuted} />}
            accessibilityLabel={ui('testBackendConnection', 'Test backend connection')}
          />
        </Section>

        {/* ── POLY-PUFF MASCOT ─────────────────────────────────────────── */}
        <Section title={`${brandName} ${ui('mascot', 'Mascot')}`} icon={<Text style={{ fontSize: 18 }}>🐡</Text>}>
          <Row
            label="Show Mascot"
            ...
            right={
              <Switch
                ...
                accessibilityRole="switch"
                accessibilityLabel={ui('showMascot', 'Show Poly-Puff mascot')}
                accessibilityState={{ checked: mascotCtx.mascotEnabled }}
              />
            }
          />
        </Section>
          ...
          <Row
            ...
            right={
              <Switch
                ...
                accessibilityRole="switch"
                accessibilityLabel={ui('soundEffects', 'Sound effects')}
          ...
          <Row
            ...
            right={
              <Switch
                ...
                accessibilityRole="switch"
                accessibilityLabel={ui('hapticFeedback', 'Haptic feedback')}
        ...
        <Section title={ui('streakReminders', 'Streak Reminders')} icon={<Bell size={18} color={C.purple || '#B06CFF'} />}>
          <Row
            ...
            desc="Get notified to keep your streak alive"
            right={
              <Switch
                value={reminderEnabled}
                onValueChange={handleReminderToggle}
                trackColor={{ false: '#D1D5DB', true: (C.purple || '#B06CFF') + '50' }}
                thumbColor={reminderEnabled ? C.purple || '#B06CFF' : C.textMuted}
                accessibilityRole="switch"
                accessibilityLabel={ui('dailyStreakReminder', 'Daily streak reminder')}
          ...
          {reminderEnabled && (
            <Row
              label={ui('reminderTime', 'Reminder Time')}
              desc={`You'll be reminded at ${formatReminderTime(reminderHour, reminderMinute)}`}
              onPress={() => setShowTimePicker(true)}
              right={
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                  <Text style={{ fontSize: scaledFont(14), fontWeight: '700', color: C.purple || '#B06CFF' }}>
                    {formatReminderTime(reminderHour, reminderMinute)}
                  </Text>
                  <ChevronRight size={14} color={C.textMuted} />
                </View>
              }
              accessibilityLabel={`Reminder time: ${formatReminderTime(reminderHour, reminderMinute)}. Tap to change.`}
              accessibilityHint="Opens time picker to choose your daily reminder time"
            />
          )}
        </Section>

        {/* ── WEEKLY DIGEST ────────────────────────────────────────────── */}
        {/* ✅ NEW v6.3: Self-contained digest settings card.               */}
        {/* Handles toggle, email input, send-now, and last-sent date.      */}
        <View style={{ marginBottom: 12 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: 4, marginBottom: 6 }}>
            <Mail size={15} color={C.cyan || '#00D9FF'} />
            <Text style={{ fontSize: scaledFont(13), fontWeight: '700', color: C.textMuted, letterSpacing: 0.5 }} accessibilityRole="header">
              WEEKLY DIGEST
            </Text>
          </View>
          <DigestSettings />
        ...
        <Section title="Offline Cache" icon={<Download size={18} color={C.amber} />}>
          <Row label="Cached Data" desc={`Rules and exercises cached for offline use (${cacheSize})`}
            right={<Text style={{ fontSize: scaledFont(13), color: C.textMuted }}>{cacheSize}</Text>}
            accessibilityLabel={`Cached data: ${cacheSize}`} />
          <Row label="Clear Cache" desc="Re-download rules and exercises next time"
            onPress={clearCache} right={<Trash2 size={16} color={C.textMuted} />}
            accessibilityLabel={ui('clearOfflineCache', 'Clear offline cache')}
            accessibilityHint="Removes cached rules and exercises. They will re-download next time." />
        </Section>

        {/* ── DATA MANAGEMENT ──────────────────────────────────────────── */}
        <Section title="Data Management" icon={<Shield size={18} color={C.red} />}>
          <Row label="Set to Default" desc="Resets language, CEFR level, and UI preferences to their defaults. Your account, progress, vaults, and bookmarks are kept."
            onPress={resetDefaults} right={<RotateCcw size={16} color={C.red} />} danger
            accessibilityLabel={ui('resetDefaults', 'Set settings to default')}
            accessibilityHint="Keeps account, progress, vaults, and bookmarks" />
          <Row label="Reset Onboarding" desc="Show the welcome walkthrough again"
            onPress={resetOnboarding} right={<RotateCcw size={16} color={C.textMuted} />}
            accessibilityLabel={ui('resetOnboarding', 'Reset onboarding walkthrough')} />
          <Row label="Clear Profile Info" desc="Remove name, profession, hobbies, photo"
            onPress={clearProfile} right={<ChevronRight size={16} color={C.textMuted} />}
            accessibilityLabel={ui('clearProfileInfo', 'Clear profile information. Language and level will be kept.')} />
          <Row label="Clear All Progress" desc="Delete exercise history, XP, badges, and streaks"
            ...
            accessibilityHint="Deletes exercise history, XP, badges, and streak data" />
        </Section>

        <Section title={wt('help')} icon={<HelpCircle size={18} color={C.cyan || '#00E5FF'} />}>
          <Row label={wt('help')} desc={wt('webapp-help-desc')}
            onPress={() => router.push('/help')} right={<ChevronRight size={16} color={C.textMuted} />}
            accessibilityLabel={wt('help')} />
          <Row label={wt('feedback')} desc={wt('webapp-feedback-desc')}
            onPress={() => router.push('/feedback')} right={<MessageSquare size={16} color={C.textMuted} />}
            accessibilityLabel={wt('feedback')} />
          <Row label={wt('ck-privacy')} desc="How we handle your data"
            onPress={() => Linking.openURL('https://polypuff.app/privacy')}
            right={<ExternalLink size={16} color={C.textMuted} />}
            accessibilityLabel={wt('ck-privacy')} />
          <Row label="Terms of Service" desc="Read Poly-Puff terms"
            onPress={() => Linking.openURL('https://polypuff.app/terms')}
            right={<ExternalLink size={16} color={C.textMuted} />}
            accessibilityLabel={ui('viewTermsExternal', 'View terms of service. Opens external link.')} />
          <Row label={wt('delete-account')} desc="Permanently remove your account and all data"
            onPress={() => Linking.openURL('https://polypuff.app/delete-account')}
            right={<ExternalLink size={16} color={C.red || '#FF4D6A'} />}
            danger
            accessibilityLabel={wt('delete-account')} />
        </Section>

        {/* ── ACCOUNT & DATA ───────────────────────────────────────────── */}
        <Section title={wt('account')} icon={<Trash2 size={18} color={C.red || '#FF4D6A'} />}>
          <Row
            label="Delete All My Data"
            desc="Permanently delete your profile, progress, and all data"
            onPress={() => {
              Alert.alert(
                '⚠️ Delete All Data',
                'This will permanently delete your profile, progress, exercise history, and all saved data. This action cannot be undone.',
                [
                  { text: t.cancel, style: 'cancel' },
                  { text: 'Delete Everything', style: 'destructive', onPress: async () => {
                    try { await AsyncStorage.clear(); Alert.alert('Data Deleted', 'All your data has been permanently removed. The app will now reset.', [{ text: 'OK' }]); }
                    catch (e) { Alert.alert('Error', 'Failed to delete data. Please try again.'); }
                  }},
                ]
              );
            }}
            right={<Trash2 size={16} color={C.red || '#FF4D6A'} />}
            danger
            accessibilityLabel={ui('deleteAllDataPermanently', 'Delete all my data permanently. This action cannot be undone.')}
            accessibilityHint="Opens a confirmation dialog to permanently delete all app data"
          />
        </Section>

        {/* ── ABOUT ────────────────────────────────────────────────────── */}
        <Section title={wt('nav-about')} icon={<Info size={18} color={C.textSec} />}>
          <Row label={ui('appVersion', 'App Version')} right={<Text style={{ fontSize: scaledFont(14), color: C.textMuted }}>{appVersion}</Text>}
            accessibilityLabel={`App version ${appVersion}`} />
          <Row label={ui('backendVersion', 'Backend Version')} right={<Text style={{ fontSize: scaledFont(14), color: C.textMuted }}>{backendVersion}</Text>}
            accessibilityLabel={`Backend version ${backendVersion}`} />
          <Row label="Grammar Rules"
            right={<Text style={{ fontSize: scaledFont(14), color: C.textMuted }}>{ruleStats.totalRules} rules · {ruleStats.categoryCount} categories</Text>}
            accessibilityLabel={`Grammar database: ${ruleStats.totalRules} rules across ${ruleStats.categoryCount} categories`} />
          <Row label="Languages Supported"
            right={<Text style={{ fontSize: scaledFont(14), color: C.textMuted }}>{ruleStats.languageCount} supported</Text>}
            accessibilityLabel={`${ruleStats.languageCount} supported languages`} />
          <Row label={ui('cefrLevels', 'CEFR Levels')} right={<Text style={{ fontSize: scaledFont(14), color: C.textMuted }}>A0 – C2</Text>}
            accessibilityLabel={ui('cefrLevels', 'CEFR levels A0 through C2')} />
        </Section>
      ...
      <Modal visible={showTimePicker} transparent animationType="fade" onRequestClose={() => setShowTimePicker(false)}>
        <TouchableOpacity style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'flex-end' }} activeOpacity={1} onPress={() => setShowTimePicker(false)}>
          <View style={{ backgroundColor: C.card, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24, paddingBottom: 40, maxHeight: '60%' }}
            accessibilityViewIsModal={true}>
            <Text style={{ fontSize: scaledFont(18), fontWeight: '800', color: C.text, marginBottom: 16, textAlign: 'center' }}
              accessibilityRole="header">{ui('chooseReminderTime', 'Choose Reminder Time')}</Text>
            <ScrollView showsVerticalScrollIndicator={false}>
              {TIME_OPTIONS.map(({ hour, minute }) => {
                const isSelected = hour === reminderHour && minute === reminderMinute;
                return (
                  <TouchableOpacity
                    key={`${hour}-${minute}`}
                    style={{
                      paddingVertical: 14, paddingHorizontal: 16, borderRadius: 10, marginBottom: 4,
                      backgroundColor: isSelected ? (C.purple || '#B06CFF') + '20' : 'transparent',
                      borderWidth: isSelected ? 1 : 0, borderColor: (C.purple || '#B06CFF') + '40',
                      minHeight: 44,
                    }}
                    onPress={() => handleTimeSelect(hour, minute)}
                    accessibilityRole="radio"
                    accessibilityLabel={formatReminderTime(hour, minute)}
                    accessibilityState={{ selected: isSelected }}
                  >
                    <Text style={{
                      fontSize: scaledFont(16), fontWeight: isSelected ? '700' : '500',
                      color: isSelected ? (C.purple || '#B06CFF') : C.text, textAlign: 'center',
                    }}>
